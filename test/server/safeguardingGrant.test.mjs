...
function makeGrant(overrides = {}) {
  return canIssueSafeguardingGrant({
    actorRole: SAFEGUARDING_ROLE,
    actorPersonId: 'officer-1',
    targetPersonId: 'student-1',
    reason: 'Immediate safeguarding concern requiring review.',
    scope: ['safeguarding', 'counselling'],
...
test('grant is time limited and scoped', () => {
  ...
  assert.equal(isSafeguardingGrantActive(grant, now), true);
  assert.equal(grantCoversScope(grant, 'safeguarding'), true);
  assert.equal(grantCoversScope(grant, 'career'), false);
  assert.equal(isSafeguardingGrantActive(grant, new Date('2026-08-23T10:16:00.000Z')), false);
});
test('grant cannot exceed one hour', () => {
  assert.equal(canIssueSafeguardingGrant({ actorRole: SAFEGUARDING_ROLE, actorPersonId: 'a', targetPersonId: 's', reason: 'Immediate safeguarding concern.', now, durationMs: 3600001 }).allowed, false);
});
test('access is denied when client merely claims safeguarding is active', () => {
  const result = decideAccess({ role: 'safeguarding_officer', relationship: 'safeguarding_officer', dataDomain: 'safeguarding', purpose: 'safeguarding', consent: 'unknown', safeguarding: 'active', timeStatus: 'active', serviceDomain: 'professional', subjectPersonId: 'student-1' });
  assert.equal(result.allowed, false);
  assert.equal(result.reason, 'safeguarding_grant_required_or_expired');
});
...
test('safeguarding grant cannot be issued to itself', () => {
  const result = canIssueSafeguardingGrant({ actorRole: SAFEGUARDING_ROLE, actorPersonId: 'same', targetPersonId: 'same', reason: 'Immediate safeguarding concern.', now });
  assert.equal(result.allowed, true); // model construction is pure; API enforces self-target prohibition.
});
