...
import { reassignRelationship, endRelationships, buildRelationshipDocument } from '../../src/security/relationshipStore.js';
...
test('reassignRelationship ends the previous professional and creates one active replacement', async () => {
  const db = makeDb([{ id: 'old', subjectPersonId: 's1', relatedPersonId: 'p1', type: 'primary_counsellor', domain: 'counselling', status: 'active' }]);
  const result = await reassignRelationship({ db, subjectPersonId: 's1', relatedPersonId: 'p2', type: 'primary_counsellor', domain: 'counselling' });
...
test('reassignRelationship is idempotent when the target is already active', async () => {
  const db = makeDb([{ id: 'current', subjectPersonId: 's1', relatedPersonId: 'p2', type: 'career_counsellor', domain: 'career', status: 'active' }]);
  const result = await reassignRelationship({ db, subjectPersonId: 's1', relatedPersonId: 'p2', type: 'career_counsellor', domain: 'career' });
