...
test('empty serialized containers do not count as report evidence', () => {
  const report = {
    scores: { riasec: {}, big5: {}, values: {}, reasoning: {}, readiness: {}, adaptability: {}, environment: {}, workEnvironment: {} },
    topCareerDirections: [], alternativeCareers: [], pathwayAnalysis: {}, pathways: [], streamAnalysis: {}, streamScenarios: [],
    educationRoadmap: {}, education: {}, skillsEvidence: {}, skillsPlan: {}, affordability: {}, friction: {}, actionRoadmap: {},
    actionPlan: {}, counsellorReview: {}, reviewLimitations: {}, careerExploration: []
  };
  const reflection = buildInstitutionCareerReflection(report);
  for (const id of ['riasec_profile','personality_profile','career_values','reasoning_profile','decision_readiness','adaptability','work_environment','top_career_directions','alternative_careers','pathway_analysis','stream_analysis','education_roadmap','skills_evidence','affordability','action_roadmap','counsellor_review']) {
...
test('interests and personality section is available when RIASEC alone is assessed', () => {
...
test('interests and personality section is available when Big Five alone is assessed', () => {
  const reflection = buildInstitutionCareerReflection({ selectedFamilyIds: ['personality'], scores: { big5: { openness: 4 } } });
  assert.equal(section(reflection, 'interest_personality').available, true);
  assert.equal(section(reflection, 'interest_personality').source, 'assessed');
});

test('interests and personality section is not assessed when neither component is assessed', () => {
