...
let sequence = 0;
function uid(prefix) { sequence += 1; return `${prefix}-${sequence}`; }
function makeRes() { return { statusCode: 200, body: null, headers: {}, status(code) { this.statusCode = code; return this; }, setHeader(key, value) { this.headers[key] = value; return this; }, json(value) { this.body = value; return this; }, end() { return this; } }; }
...
async function createCoordinatorInstitution({ name, licenseCount = 150, paymentStatus = 'paid' } = {}) {
  ...
  await db.collection('institutions').doc(institutionId).set({ id: institutionId, name, tenantCode: name.replace(/[^A-Za-z0-9]/g, '').slice(0, 12).toUpperCase(), institutionCode: `SSZ-TEST-${uid('code').toUpperCase()}`, status: 'active', contactEmail: coordinator.email, coordinator: { uid: coordinator.uid, email: coordinator.email, role: 'coordinator' }, licenses: { purchased: licenseCount, used: 0, available: licenseCount, paymentStatus, pricePerLicense: 0, totalAmount: 0 }, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
  await db.collection('users').doc(coordinator.uid).set({ role: 'institution_member', institutionId, institutionName: name, institutionRole: 'coordinator', status: 'active' }, { merge: true });
  await getAdminAuth().setCustomUserClaims(coordinator.uid, { role: 'institution_member', institutionId, institutionRole: 'coordinator' });
...
before(() => { assert.equal(isEmulatorMode(), true, 'Institution integration tests require Firebase emulators.'); assert.ok(process.env.FIREBASE_AUTH_EMULATOR_HOST); assert.ok(process.env.FIRESTORE_EMULATOR_HOST); });
after(async () => { try { await signOut(clientAuth); } catch (_) {} });

test('150-license institutional journey: provision → redeem → assess → report', async () => {
  const { coordinator, institutionId, db } = await createCoordinatorInstitution({ name: '150 License Integration School', licenseCount: 150, paymentStatus: 'paid' });
  const parentEmail = `parent-${uid('test')}@emulator.test`;
  const rows = Array.from({ length: 150 }, (_, index) => ({ fullName: `Institution Test Student ${String(index + 1).padStart(3, '0')}`, className: index < 50 ? '10' : index < 100 ? '11' : '12', section: ['A', 'B', 'C'][index % 3], rollNumber: String(index + 1), ...(index === 0 ? { parentName: 'Integration Parent', parentEmail } : {}) }));

  const provision = await call(provisionRoster, { token: coordinator.idToken, body: { institutionId, rows } });
  ...
  assert.equal(provision.body.parents.length, 1);
  assert.equal(provision.body.parents[0].email, parentEmail);
  assert.equal(provision.body.parents[0].name, 'Integration Parent');
  assert.equal(provision.body.parents[0].created, true);

  const parentUid = provision.body.parents[0].uid;
  const parentBeforeClaim = await db.collection('users').doc(parentUid).get();
  assert.equal(parentBeforeClaim.data().role, 'parent');
  assert.deepEqual(parentBeforeClaim.data().linkedStudentIds, [], 'parent can exist before student account creation');
  assert.ok(parentBeforeClaim.data().linkedRosterIds.length >= 1);

  const students = provision.body.students;
  const codes = students.map(student => student.accessCode);
  assert.equal(new Set(codes).size, 150);
  assert.ok(codes.every(code => /^SSZ-[A-Z0-9]+-\d{2}-[A-F0-9]{12}$/.test(code)));

  const dashboard = await call(institutionDashboard, { token: coordinator.idToken, method: 'GET', query: { institutionId } });
  ...
  const parentAfterClaim = await db.collection('users').doc(parentUid).get();
  assert.deepEqual(parentAfterClaim.data().linkedStudentIds, [studentA.uid]);
  assert.equal(parentAfterClaim.data().childRelationships[studentA.uid], 'guardian');
  const studentAfterClaim = await db.collection('users').doc(studentA.uid).get();
  assert.equal(studentAfterClaim.data().parentUid, parentUid);
  assert.equal(studentAfterClaim.data().parentEmail, parentEmail);
  assert.equal(studentAfterClaim.data().guardianRelationships[parentUid], 'guardian');

  const replayBySameStudent = await call(redeemCode, { token: studentA.idToken, body: { code } });
  assert.equal(replayBySameStudent.status, 200);
  ...
  await db.collection('users').doc(studentA.uid).set({ careerAssessmentV2: { version: 'integration-test', scores: { riasecCode: 'RIA', readinessPercent: 82 }, careerExploration: [{ id: 'career-1', name: 'Integration Test Career', category: 'Test' }] } }, { merge: true });
  await institutionRef.collection('roster').doc(rosterId).set({ assessmentStatus: 'completed', reportStatus: 'ready', updatedAt: new Date().toISOString() }, { merge: true });

  const report = await call(studentReport, { token: coordinator.idToken, method: 'GET', query: { institutionId, rosterId } });
  ...
  const finalDashboard = await call(institutionDashboard, { token: coordinator.idToken, method: 'GET', query: { institutionId } });
...
test('institutional code cannot be redeemed while entitlement is unpaid', async () => {
  const { coordinator, institutionId, db } = await createCoordinatorInstitution({ name: 'Unpaid Integration School', licenseCount: 1, paymentStatus: 'paid' });
  const provision = await call(provisionRoster, { token: coordinator.idToken, body: { institutionId, rows: [{ fullName: 'Locked Student', className: '10', section: 'A', rollNumber: '1' }] } });
  assert.equal(provision.status, 200, JSON.stringify(provision.body));
  await db.collection('institutions').doc(institutionId).set({ licenses: { paymentStatus: 'pending' }, updatedAt: new Date().toISOString() }, { merge: true });
  const student = await mintToken({});
  const redemption = await call(redeemCode, { token: student.idToken, body: { code: provision.body.students[0].accessCode } });
