...
function VideosContent() {
  ...
  const channelUrl = channel?.url || 'https://www.youtube.com/@secretsharz8427';
    ...
    <>
      <section className="ss-video-hero" aria-labelledby="video-page-title">
        <div className="ss-video-container">
          ...
          <p className="ss-video-lead">Explore videos published on the official Secret Sharz channel. New public uploads appear here automatically.</p>
        ...
        <div className="ss-video-container">
          <div className="ss-video-heading-row">
            <div>
              ...
              <h2 id="video-library-title">All Secret Sharz videos</h2>
            </div>
            {channel?.title ? <span className="ss-video-source">Live source: {channel.title}</span> : null}
          </div>
              ...
              <div className="ss-video-grid">
                {items.map((video) => (
                  <article className="ss-video-card" key={video.videoId}>
                    <button type="button" className="ss-video-thumbnail" onClick={() => setSelected(video)} aria-label={`Play video: ${video.title}`}>
                    ...
                    <div className="ss-video-card-body">
                      <h3>{video.title}</h3>
                      <p className="ss-video-meta">{formatDate(video.publishedAt)}{video.duration ? ` · ${formatDuration(video.duration)}` : ''}</p>
        ...
        <div className="ss-video-dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}>
          <section className="ss-video-dialog" role="dialog" aria-modal="true" aria-labelledby="selected-video-title" tabIndex="-1">
            ...
            <div className="ss-video-player">
              <iframe src={`${selected.embedUrl}?rel=0&cc_load_policy=1`} title={`YouTube video: ${selected.title}`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" />
            </div>
      ...
      <style jsx>{`
        .ss-video-page{min-height:100vh;background:#FDFCFA;color:#1E2820}.ss-video-container{width:min(1180px,calc(100% - 40px));margin:0 auto}.ss-video-hero{padding:92px 0 76px;background:linear-gradient(135deg,#F7F3ED 0%,#FDFCFA 62%,#EBF4EE 100%)}
        .ss-video-eyebrow{margin:0 0 12px;color:#4A7C59;font-size:12px;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.ss-video-hero h1{max-width:820px;margin:0;font-family:Fraunces,serif;font-size:clamp(40px,6vw,68px);line-height:1.05;letter-spacing:-1.5px}.ss-video-hero h1 em{color:#4A7C59;font-weight:600}.ss-video-lead{max-width:720px;margin:24px 0;color:#3D4A40;font-size:18px;line-height:1.7}.ss-video-channel-link,.ss-video-watch-link{color:#24583a;font-weight:800;text-decoration:underline;text-underline-offset:3px}
        .ss-video-library{padding:72px 0}.ss-video-heading-row{display:flex;align-items:end;justify-content:space-between;gap:20px;margin-bottom:28px}.ss-video-heading-row h2{margin:0;font-family:Fraunces,serif;font-size:40px}.ss-video-source{color:#66736b;font-size:13px}.ss-video-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:24px}.ss-video-card{background:#fff;border:1px solid #DCE3DE;border-radius:18px;overflow:hidden;box-shadow:0 4px 16px rgba(30,40,32,.05)}
        .ss-video-thumbnail{display:block;position:relative;width:100%;aspect-ratio:16/9;border:0;padding:0;background:#EAF0EB;cursor:pointer;overflow:hidden}.ss-video-thumbnail img{width:100%;height:100%;display:block;object-fit:cover}.ss-video-play{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:56px;height:56px;border-radius:50%;background:#2E6B4A;color:#fff;display:grid;place-items:center;font-size:20px;padding-left:3px;box-shadow:0 10px 30px rgba(0,0,0,.2)}.ss-video-card-body{padding:18px}.ss-video-card h3{margin:0 0 8px;font-size:17px;line-height:1.35}.ss-video-meta{margin:0 0 10px;color:#66736b;font-size:12px;font-weight:700}.ss-video-description{margin:0 0 14px;color:#56635b;font-size:13px;line-height:1.55;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}
        .ss-video-load-more{text-align:center;margin-top:34px}.ss-video-load-more button,.ss-video-state button{min-height:48px;border:0;border-radius:12px;padding:12px 20px;background:#2E6B4A;color:#fff;font-weight:800;cursor:pointer}.ss-video-load-more button:disabled{opacity:.6;cursor:wait}.ss-video-state{padding:40px;background:#F7F3ED;border:1px solid #DCE3DE;border-radius:18px;color:#3D4A40}.ss-video-state strong{display:block;margin-bottom:6px}.ss-video-error p{margin:0 0 16px}
        ...
        :where(.ss-video-thumbnail,.ss-video-load-more button,.ss-video-state button,.ss-video-dialog-heading button,.ss-video-watch-link,.ss-video-channel-link):focus-visible{outline:3px solid #1F6B46;outline-offset:3px}@media(max-width:900px){.ss-video-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.ss-video-hero{padding:72px 0 56px}}@media(max-width:620px){.ss-video-container{width:min(100% - 28px,1180px)}.ss-video-grid{grid-template-columns:1fr}.ss-video-heading-row{align-items:start;flex-direction:column}.ss-video-heading-row h2{font-size:32px}.ss-video-dialog-backdrop{padding:10px}.ss-video-dialog{padding:12px}}@media(prefers-reduced-motion:reduce){.ss-video-play{transform:translate(-50%,-50%)}}
      `}</style>
    </>
  );
}
...
export default function VideosPage() {
  return (
    <PublicShell
      title="Secret Sharz Videos | Official YouTube Library"
      description="Explore the live public video library from the official Secret Sharz YouTube channel."
    >
      {() => <VideosContent />}
    </PublicShell>
  );
