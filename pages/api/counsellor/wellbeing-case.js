...
export default async function handler(req, res) {
  ...
  try {
    ...
    const consentSnap = await db.collection('consents').where('studentAuthUid','==',identity.authUid || identity.documentId).where('service','==','wellbeing').where('status','==','active').limit(1).get();
    const consent = { required:true, status: consentSnap.empty ? 'missing' : 'active' };
    assertAccess({ actor, studentId:identity.authUid, institutionId, service:'wellbeing', domain:'counselling', relationship:actorAssignment, consent, safeguarding:{ restricted:false } });
