...
export default async function handler(req,res){
  ...
  const canonicalId=authResult.authUid||authResult.studentId;
  const studentRef=db.collection('students').doc(canonicalId);
  if(req.method==='POST'){
    ...
    const noteRef=await studentRef.collection('career_notes').add({providerId:authResult.viewerId,type,title,content,followUpDate,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
    return res.status(201).json({ok:true,id:noteRef.id,studentId:authResult.studentId,ssStudentId:authResult.studentId});
  }
  ...
  return res.status(200).json({student:{...resolved.profile,studentId:authResult.studentId,ssStudentId:authResult.studentId,authUid:authResult.authUid},notes:sortNewest(notesSnap.docs.map(doc=>({id:doc.id,...doc.data()}))),roadmaps:sortNewest(roadmapsSnap.docs.map(doc=>({id:doc.id,...doc.data()}))) });
}
