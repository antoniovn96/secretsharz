...
function buildCareerDetail(rawStudent, studentId, allUsers = new Map(), institution = null) {
  ...
  return {
    ...
    institution: {
      id: profile.institution?.id || academic.institutionId || '',
      name: institution?.name || profile.institution?.name || academic.institutionName || '',
      academicYear: profile.institution?.academicYear || academic.academicYear || '',
      enrollmentStatus: profile.institution?.enrollmentStatus || rawStudent.enrollmentStatus || 'active',
    },
    academic: {
    ...
    assignment: {
      ...
      professionalName: assignedProfessional?.name || assignedProfessional?.fullName || '',
    },
    guardians: {
      count: profile.family?.guardians?.length || 0,
      relationships: (profile.family?.guardians || []).map(guardian => ({ relationship: guardian.relationship || 'guardian', name: guardian.name || '', consentStatus: guardian.consentStatus || null })),
    },
    assessment: {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCareerDirectoryRequest(req)) {
      ...
      if (!resolved.allowed || resolved.role !== 'super_admin') {
        return res.status(403).json({ error: 'Super Admin access required for the career directory detail.' });
      }
      const usersSnapshot = await db.collection('users').get();
      const allUsers = new Map(usersSnapshot.docs.map(doc => {
        const raw = doc.data() || {};
        return [doc.id, { id: doc.id, name: raw.name || raw.fullName || raw.studentProfile?.identity?.fullName || '' }];
      }));
      ...
      if (institutionId) {
        ...
        if (institutionSnap.exists) institution = { id: institutionSnap.id, ...institutionSnap.data() };
      }
