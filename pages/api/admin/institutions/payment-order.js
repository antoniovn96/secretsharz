...
export default async function handler(req,res){
  ...
  try{
    ...
    const amount=Math.round(Number(institution.licenses?.totalAmount||0));if(!Number.isInteger(amount)||amount<0)return res.status(400).json({error:'Institution has an invalid payable amount.'});
    if(amount===0){const now=new Date().toISOString();await institutionRef.set({status:'active',licenses:{paymentStatus:'paid',paidAt:now,paidBy:decoded.uid,paidAmount:0,currency:'INR'},updatedAt:now},{merge:true});return res.status(200).json({sponsored:true,verified:true,institutionId,amount:0});}
    ...
    const payload={amount,currency:'INR',receipt,notes:{institutionId,product:'institution_assessment_license',licenseCount:String(institution.licenses?.purchased||0),createdBy:decoded.uid}};
    ...
    await db.collection('institutionPayments').doc(data.id).set({orderId:data.id,institutionId,institutionName:institution.name||'',amountPaise:amount,currency:'INR',licenseCount:Number(institution.licenses?.purchased||0),status:'created',receipt,createdBy:decoded.uid,createdAt:new Date().toISOString()});
    return res.status(200).json({orderId:data.id,amount:data.amount,currency:data.currency,keyId,institutionId,licenseCount:Number(institution.licenses?.purchased||0),institutionName:institution.name||''});
