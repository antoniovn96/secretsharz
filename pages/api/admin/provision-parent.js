...
export default async function handler(req,res){
  ...
  const studentIds=Array.isArray(req.body?.studentIds)?req.body.studentIds.filter(Boolean).slice(0,50):[];
  ...
  try{
    const result=await provisionParentAccount({adminAuth:getAdminAuth(),adminDb:getAdminFirestore(),parentName:name,parentEmail:email,institutionId,institutionName,provisioningMethod:role,studentIds,relationship});
    try{await getAdminFirestore().collection('auditEvents').add({actorUid:decoded.uid||null,actorEmail:decoded.email||null,actorRole:role,targetUid:result.uid,targetRole:'parent',action:'provision_parent',institutionId,studentIds,relationship,createdAt:new Date().toISOString()});}catch(auditError){console.error('[provision-parent] audit write failed:',auditError?.message||auditError);}
