...
export default async function handler(req, res) {
  ...
  if (!isFounder && !hasInstitutionAccess) return res.status(403).json({ error: 'Institution coordinator access required.' });
  const institutionRef = db.collection('institutions').doc(institutionId);
  const institution = await institutionRef.get(); if (!institution.exists) return res.status(404).json({ error: 'Institution not found.' });
  ...
  const roster = await institutionRef.collection('roster').doc(rosterId).get(); if (!roster.exists) return res.status(404).json({ error: 'Student record not found.' });
  const rosterData = roster.data();
  if (!rosterData.claimedBy) return res.status(409).json({ error: 'This student has not yet claimed the assessment code.' });
  if (rosterData.assessmentStatus !== 'completed' && rosterData.reportStatus !== 'ready') return res.status(409).json({ error: 'The student career assessment has not been completed.' });
  const studentSnap = await db.collection('users').doc(rosterData.claimedBy).get(); if (!studentSnap.exists) return res.status(404).json({ error: 'Student account not found.' });
  const student = studentSnap.data(); const careerReport = student.careerAssessmentV2;
  if (!careerReport) return res.status(409).json({ error: 'Assessment report is not ready yet.' });
  const coverage = getInstitutionCareerReportStatus(careerReport);
  const assessmentEvidence = getAssessmentEvidenceCoverage(careerReport);
  return res.status(200).json({ student: { fullName: rosterData.fullName || '', className: rosterData.className || '', section: rosterData.section || '', rollNumber: rosterData.rollNumber || '' }, report: careerReport, coverage, assessmentEvidence, dataScope: 'institutional_career_guidance' });
}
