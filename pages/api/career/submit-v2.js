...
export default async function handler(req,res){
 ...
 const bundle=resolveBundle(safeString(body.bundleId,160));
 ...
 const codeId=safeString(intake.licenseCode||userData.institutionAccessCode,120);
 let institutionEntitled=false;let institutionCodeRecord=null;
 if(codeId){try{const s=await db.collection('institutionCodes').doc(codeId).get();if(s.exists){institutionCodeRecord=s.data();institutionEntitled=s.data()?.status==='redeemed'&&s.data()?.redeemedBy===decoded.uid;}}catch(_){}}
