...
export default async function handler(req, res) {
  ...
  const report = {
    ...
    intake: {
      ...
      className: safeString(intake.className, 100),
      stream: safeString(intake.stream, 100),
      institutionName: safeString(intake.institutionName || userData.institutionName, 160),
      likedSubjects: Array.isArray(intake.likedSubjects) ? intake.likedSubjects.slice(0, 30) : [],
      dislikedSubjects: Array.isArray(intake.dislikedSubjects) ? intake.dislikedSubjects.slice(0, 30) : [],
      hobbies: safeString(intake.hobbies, 500),
      curiosity: safeString(intake.curiosity, 500),
      goal: safeString(intake.goal, 500),
    ...
    reflection: {
      ...
      recommendedNextStep: pathway === 'working_professional'
        ? 'Review the available stay/grow, lateral pivot and industry pivot pathways with a career professional.'
        : 'Explore at least three pathways and compare their education, work, skills and lived experience before deciding.',
    },
  };
  ...
  try {
    ...
    await assessmentRef.set(assessmentRecord);
    await careerProfileRef.set({
      authUid: decoded.uid,
      ssStudentId,
      version: ASSESSMENT_VERSION,
