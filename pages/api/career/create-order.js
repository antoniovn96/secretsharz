...
export default async function handler(req, res) {
  ...
  const customerType = customerTypeForProduct(requestedProduct);
  ...
  if (couponCode) {
    ...
    if (!couponSnap.exists) return jsonError(res, 400, 'Coupon code was not found.');
    coupon = couponSnap.data();
    const validation = validateCoupon(coupon, { productKey: requestedProduct, customerType, quantity });
    if (!validation.valid) return jsonError(res, 400, validation.reason);
    discountAmount = calculateCouponDiscount(coupon, baseAmount, quantity);
  }

  const finalAmount = Math.max(0, baseAmount - discountAmount);
  const now = new Date().toISOString();
  const paymentOrderRef = db.collection('paymentOrders').doc();
  const commonRecord = {
    ...
    status: finalAmount === 0 ? 'sponsored' : 'created',
    createdAt: now,
    updatedAt: now,
  };
  ...
  if (finalAmount === 0) {
    await db.runTransaction(async (transaction) => {
      ...
      if (couponCode) {
        if (!freshCoupon?.exists) throw Object.assign(new Error('Coupon is no longer available.'), { status: 409 });
        const fresh = freshCoupon.data();
        const validation = validateCoupon(fresh, { productKey: requestedProduct, customerType, quantity });
        if (!validation.valid) throw Object.assign(new Error(validation.reason), { status: 409 });
        transaction.update(freshCoupon.ref, { redemptions: Number(fresh.redemptions || 0) + 1, updatedAt: now });
      }
      transaction.set(paymentOrderRef, {
        ...commonRecord,
        paymentId: null,
