...
import { calculateCouponDiscount, customerTypeForProduct, getCareerProductConfig, normaliseCouponCode, validateCoupon, serialiseCoupon } from '../../../src/server/careerPricing.js';
...
export default async function handler(req,res){
  ...
  const db=getAdminFirestore(); const requestedProduct=String(req.body?.product||'student_individual').trim();
  const product=await getCareerProductConfig(db,requestedProduct); if(!product)return jsonError(res,400,'Unsupported career product.'); if(product.active===false)return jsonError(res,409,'This career product is currently unavailable.'); if(product.amountPaise===0)return jsonError(res,400,'This product does not require a payment order.');
  ...
  if(finalAmount===0){
    await db.runTransaction(async transaction=>{
      ...
      const access={status:'paid',product:product.sku,productKey:requestedProduct,orderId:paymentOrderRef.id,paymentId:null,amount:0,currency:'INR',couponCode,sponsored:true,paidAt:now,reportTier:product.reportTier};
      transaction.set(db.collection('users').doc(decoded.uid),requestedProduct==='student_games_pro'?{gamesAccess:access}:{careerReportAccess:access},{merge:true});
    });
    return res.status(200).json({sponsored:true,verified:true,access:'paid',product:product.sku,productKey:requestedProduct,orderId:paymentOrderRef.id,amount:0});
  }
  ...
  try{
    const receipt=`career_${product.sku.toLowerCase()}_${decoded.uid.slice(0,12)}_${Date.now()}`; const payload={amount:finalAmount,currency:'INR',receipt,notes:{uid:decoded.uid,product:product.sku,productKey:requestedProduct,reportTier:product.reportTier,coupon:couponCode||''}};
