...
import {ASSESSMENT_SOURCE_LABELS} from './AssessmentEvidenceBadge';
const card={background:'#fff',border:'1px solid #e2e8f0',borderRadius:18,padding:20,boxShadow:'0 8px 28px rgba(15,23,42,.05)'};
const SOURCE_TONES={assessed:['#f0fdf4','#15803d'],partially_assessed:['#eff6ff','#1d4ed8'],derived_from_assessment:['#eff6ff','#1d4ed8'],career_catalogue:['#fffbeb','#a16207'],not_assessed:['#fff7ed','#c2410c'],unavailable:['#f8fafc','#64748b']};
export default function InstitutionCareerReportCoverage({report={}}){
 ...
 const rows=STUDENT_PREMIUM_REPORT.map(section=>{const row=byId.get(section.id);const source=row?.source||'unavailable';return {...section,source,status:ASSESSMENT_SOURCE_LABELS[source]||ASSESSMENT_SOURCE_LABELS.unavailable,tone:source==='unavailable'?'missing':source==='not_assessed'?'not_assessed':'available'};});
 const available=rows.filter(x=>x.tone==='available').length;
 const assessed=rows.filter(x=>x.source==='assessed').length;
 const partial=rows.filter(x=>x.source==='partially_assessed').length;
 ...
 return <section style={{...card,marginTop:14}}>
  ...
  <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12,fontSize:11,fontWeight:900}}><span>Assessed: {assessed}</span><span>Partial: {partial}</span><span>Derived: {derived}</span><span>Catalogue: {catalogue}</span><span>Not assessed: {notAssessed}</span><span>Not available: {unavailable}</span></div>
