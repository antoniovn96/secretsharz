...
const SOURCE_LABELS={assessed:'Assessed',derived_from_assessment:'Derived from assessment evidence',career_catalogue:'Career catalogue information',unavailable:'Not available'};
const SOURCE_TONES={assessed:['#f0fdf4','#15803d'],derived_from_assessment:['#eff6ff','#1d4ed8'],career_catalogue:['#fffbeb','#a16207'],unavailable:['#f8fafc','#64748b']};
export default function InstitutionCareerReportCoverage({report={}}){
 ...
 const rows=STUDENT_PREMIUM_REPORT.map(section=>{const row=byId.get(section.id);const source=row?.source||'unavailable';return {...section,source,status:SOURCE_LABELS[source]||SOURCE_LABELS.unavailable,tone:source==='unavailable'?'missing':'available'};});
 ...
 return <section style={{...card,marginTop:14}}>
  ...
  <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12,fontSize:11,fontWeight:900}}><span>Assessed: {assessed}</span><span>Derived: {derived}</span><span>Catalogue: {catalogue}</span><span>Unavailable: {rows.length-available}</span></div>
