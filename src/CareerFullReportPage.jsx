...
function normalise(data) {
  ...
  return {
    ...
    profile: data,
    bundle: {
      title: assessment.bundleTitle || assessment.bundleSku || assessment.bundleId || 'Career Assessment',
    },
    intake: assessment.intake || {},
    scores: assessment.scores || {},
    careerExploration: Array.isArray(data?.careerExploration) ? data.careerExploration : [],
    reportTier: assessment.reportTier || 'full',
  };
}
  ...
  return (
    <div style={{ display: 'grid', gap: 10 }}>
      {items.slice(0, 8).map((career, index) => (
        <div key={`${career.id || career.name || career.title || 'career'}-${index}`} style={{ padding: 15, background: '#f8fafc', borderRadius: 12, border: '1px solid #eef2f7' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 15 }}>
            <strong>{index + 1}. {career.name || career.title}</strong>
            {career.explorationIndex != null && <strong style={{ color: '#4f46e5' }}>{career.explorationIndex}%</strong>}
          </div>
...
function Content({ id, report }) {
  ...
  switch (id) {
    ...
    case 'interest_personality':
      return <><p style={muted}>Career interests are available below. Personality results are only interpreted when the Personality family was actually completed.</p><div style={{ fontSize: 40, fontWeight: 950, letterSpacing: 7, color: '#4f46e5' }}>{valueOr(scores.riasecCode, '—')}</div>{!scores.big5 && notAssessed}</>;
    case 'riasec_profile':
    ...
    case 'developmental_context':
      return <><p><b>Education stage:</b> {valueOr(intake.educationStage)}</p><p><b>Class / year:</b> {valueOr(intake.className)}</p><p><b>Stream:</b> {valueOr(intake.stream)}</p><p><b>Subjects:</b> {valueOr(intake.subjectsLiked || intake.likedSubjects)}</p><p><b>Academic average:</b> {valueOr(intake.academicAverage)}</p></>;
    ...
    case 'affordability':
      return <><p style={muted}>This section requires richer evidence than the stored assessment contains. The system deliberately avoids inventing personalised pathway, skills, affordability or scholarship conclusions.</p>{notAssessed}</>;
    ...
    async function loadReport() {
      try {
        ...
        if (!cancelled) {
          ...
          setIdentity(getProfileIdentity(auth.currentUser, {
            name: data?.identity?.name,
            preferredName: data?.identity?.preferredName,
    ...
    <div style={{ background: '#f8fafc', minHeight: '100vh', padding: '28px 18px 70px' }}>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto' }}>
        <section style={{ ...card, background: 'linear-gradient(135deg,#0f172a,#1e293b)', color: '#fff', display: 'flex', alignItems: 'center', gap: 18 }}>
          ...
          <div><div style={{ color: '#fbbf24', fontSize: 11, fontWeight: 900, letterSpacing: 1.5 }}>VIDYAVANTAGE · FULL CAREER INTELLIGENCE</div><h1 style={{ fontSize: 34, margin: '9px 0' }}>{identity.name} Career Intelligence Report</h1><p style={{ color: '#cbd5e1', lineHeight: 1.7, margin: 0 }}>Integrated report · evidence-conscious · no invented psychometric results</p></div>
        </section>
        <section style={{ ...card, marginTop: 16, background: '#fffbeb' }}><b>Integrated assessment bundle</b><p style={{ ...muted, marginBottom: 0 }}>This report is generated from the canonical Career profile. Additional modules are marked Not assessed rather than being fabricated.</p></section>
