...
function Content({ id, report }) {
  ...
  switch (id) {
    ...
    case 'interest_personality':
      return (
        <>
          <p style={muted}>Career interests are available below. Personality results are only interpreted when the Personality family was actually completed.</p>
          <div style={{ fontSize: 40, fontWeight: 950, letterSpacing: 7, color: '#4f46e5' }}>{valueOr(scores.riasecCode, '—')}</div>
          {report.legacyAssessment && notAssessed}
        </>
      );

    case 'riasec_profile':
      return riasec.length ? riasec.map(([key, score]) => (
        <div key={key} style={{ margin: '12px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800, fontSize: 12 }}><span>{key}</span><span>{score}%</span></div>
          <div style={{ height: 9, background: '#e2e8f0', borderRadius: 99, marginTop: 5 }}>
            <div style={{ width: `${Math.min(100, Number(score))}%`, height: '100%', background: 'linear-gradient(90deg,#4f46e5,#7c3aed)', borderRadius: 99 }} />
          </div>
        </div>
      )) : notAssessed;
  ...
  useEffect(() => {
    ...
    async function loadReport() {
      try {
        ...
        if (!cancelled) setReport(normalise(data));
      } catch (err) {
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', padding: '28px 18px 70px' }}>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto' }}>
        <section style={{ ...card, background: 'linear-gradient(135deg,#0f172a,#1e293b)', color: '#fff' }}>
          <div style={{ color: '#fbbf24', fontSize: 11, fontWeight: 900, letterSpacing: 1.5 }}>VIDYAVANTAGE · FULL CAREER INTELLIGENCE</div>
          <h1 style={{ fontSize: 34, margin: '9px 0' }}>{report.profile?.name || 'Your'} Career Intelligence Report</h1>
          <p style={{ color: '#cbd5e1', lineHeight: 1.7 }}>20-section integrated report architecture · evidence-conscious · no invented psychometric results</p>
        </section>

        <section style={{ ...card, marginTop: 16, background: '#fffbeb' }}>
          <b>{report.legacyAssessment ? 'Earlier assessment detected' : 'Integrated assessment bundle'}</b>
          <p style={{ ...muted, marginBottom: 0 }}>{report.legacyAssessment ? 'This account was created with the earlier RIASEC-only assessment. The new report structure is active, but additional modules are marked Not assessed rather than being fabricated.' : 'Your selected assessment families are combined into one integrated Career Intelligence report.'}</p>
        </section>

        {pages.map((page, index) => (
          <section className="report-page" key={page.id} style={{ ...card, marginTop: 16, minHeight: 620 }}>
            <div style={{ fontSize: 11, fontWeight: 900, letterSpacing: 1.4, color: '#4f46e5' }}>{String(index + 1).padStart(2, '0')} / {pages.length}</div>
            <h2 style={{ margin: '7px 0 16px', fontSize: 27, color: '#0f172a' }}>{page.title}</h2>
            <Content id={page.id} report={report} />
          </section>
        ))}
      </main>
    </div>
