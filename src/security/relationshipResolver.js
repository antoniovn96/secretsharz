...
const candidateTypeForDomain = Object.freeze({
  career: ['career_counsellor'],
  counselling: ['primary_counsellor'],
...
export async function resolveRelationship({ db, subjectPersonId, relatedPersonId, domain, type }) {
  ...
  if (type) {
    ...
    // Never revive an ended/suspended/revoked canonical relationship from legacy fields.
    const snapshot = await db.collection(RELATIONSHIP_COLLECTION)
  ...
  const student = await db.collection('students').doc(subjectPersonId).get();
  if (!student.exists) return { allowed: false, source: RESOLUTION_SOURCES.NONE, relationship: null };
  const { candidates } = buildLegacyRelationshipCandidates(subjectPersonId, student.data() || {});
