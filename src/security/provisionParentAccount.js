...
export async function provisionParentAccount({adminAuth,adminDb,parentName,parentEmail,institutionId=null,institutionName='',rosterIds=[],studentIds=[],provisioningMethod='admin',relationship='guardian',allowUnlinked=false}){
  ...
  const knownInstitutionIds=Array.from(new Set([...existingInstitutionIds,...(existingProfile.institutionId?[existingProfile.institutionId]:[])]));
  if(institutionId&&knownInstitutionIds.some((id)=>id!==institutionId))throw new Error('Parent account is already linked to another institution.');
  ...
  const mergedInstitutionIds=Array.from(new Set([...knownInstitutionIds,...(institutionId?[institutionId]:[])]));
  ...
  for(const studentId of studentIds){
    ...
    await adminDb.runTransaction(async transaction=>{
      ...
      const studentInstitutionId=student.academic?.current?.institutionId||student.academic?.institutionId||student.institution?.id||student.institutionId||null;
      if(institutionId&&studentInstitutionId&&studentInstitutionId!==institutionId)throw new Error('Student is already linked to another institution.');
