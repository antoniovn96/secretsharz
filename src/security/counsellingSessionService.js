...
function canonicalSession(data, id) {
  return {
    id,
...
function legacySession(data, id) {
  return {
    id: `legacy:${id}`,
    legacyId: id,
    studentAuthUid: data.studentAuthUid || null,
    providerId: data.providerId || null,
    institutionId: data.institutionId || null,
    domain: data.domain || 'counselling',
...
export async function getCounsellingSessions({ db, studentId, professionalId }) {
  ...
  const canonicalSnap = await db.collection(COLLECTION)
    .where('studentAuthUid', '==', studentId)
    .orderBy('createdAt', 'desc')
    .limit(100)
    .get();

  const legacySnap = await db.collection('students').doc(studentId).collection(LEGACY_SUBCOLLECTION)
    .orderBy('createdAt', 'desc')
    .limit(100)
    .get();

  const canonical = canonicalSnap.docs.map((doc) => canonicalSession(doc.data(), doc.id));
  const legacy = legacySnap.docs.map((doc) => legacySession(doc.data(), doc.id));
