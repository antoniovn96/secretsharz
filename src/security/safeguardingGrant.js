...
// ordinary consent-based access. It is NOT created by trusting a client
// supplied `safeguarding: active` flag.

export const SAFEGUARDING_ROLE = 'safeguarding_officer';
export const SAFEGUARDING_PURPOSE = 'safeguarding';
export const SAFEGUARDING_GRANT_STATUS = Object.freeze([
  'active',
  'expired',
  'revoked',
]);

export const SAFEGUARDING_SCOPES = Object.freeze([
  'safeguarding',
  'counselling',
  'sen',
  'career',
...
const MAX_DURATION_MS = 60 * 60 * 1000; // one hour maximum per grant
...
export function isSafeguardingRole(role) {
  return role === SAFEGUARDING_ROLE;
}
...
export function normalizeSafeguardingGrantInput(input = {}) {
  ...
  if (!Array.isArray(scope) || scope.length === 0 || scope.some((value) => !SAFEGUARDING_SCOPES.includes(value))) {
    throw new Error('A valid safeguarding scope is required.');
  }

  const start = new Date(issuedAt);
  const end = new Date(expiresAt);
  if (!Number.isFinite(start.getTime()) || !Number.isFinite(end.getTime())) {
    throw new Error('issuedAt and expiresAt must be valid timestamps.');
  }
  if (end <= start) throw new Error('expiresAt must be after issuedAt.');
  if (end.getTime() - start.getTime() > MAX_DURATION_MS) {
    throw new Error('Safeguarding grants cannot exceed one hour.');
  }
...
export function isSafeguardingGrantActive(grant, now = new Date()) {
  ...
  return Number.isFinite(current) && Number.isFinite(issued) && Number.isFinite(expires)
    && current >= issued && current < expires;
}

export function grantCoversScope(grant, requestedScope) {
  return isSafeguardingGrantActive(grant)
    && typeof requestedScope === 'string'
    && grant.scope.includes(requestedScope);
}
...
export function canIssueSafeguardingGrant({ actorRole, actorPersonId, targetPersonId, reason, now = new Date(), durationMs = 15 * 60 * 1000, scope = ['safeguarding'] }) {
  ...
  assertNonEmptyString(actorPersonId, 'actorPersonId');
  assertNonEmptyString(targetPersonId, 'targetPersonId');
  assertNonEmptyString(reason, 'reason');
  if (!Array.isArray(scope) || scope.length === 0 || scope.some((value) => !SAFEGUARDING_SCOPES.includes(value))) {
  ...
  const grant = normalizeSafeguardingGrantInput({
    grantId: `sg_${issuedAt.getTime()}_${targetPersonId}`,
