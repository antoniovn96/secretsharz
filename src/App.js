...
const CSS = `
  :root {
    ...
    --shadow-lg:0 20px 60px rgba(30,40,32,0.13);--r-sm:14px;--r-md:22px;--r-lg:32px;--r-xl:48px;
  }
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .anim-up{animation:floatUp 0.7s ease both;}.anim-up-1{animation:floatUp 0.7s 0.1s ease both;}
  .anim-up-2{animation:floatUp 0.7s 0.2s ease both;}.anim-up-3{animation:floatUp 0.7s 0.35s ease both;}
  ...
  .fc-icon{font-size:28px;margin-bottom:10px;}.fc-label{font-size:11px;font-weight:700;color:var(--muted);letter-spacing:1px;text-transform:uppercase;margin-bottom:4px;}
  ...
  .pillar-card{background:white;border-radius:var(--r-md);padding:36px 32px;border:1.5px solid var(--border);box-shadow:var(--shadow-sm);transition:all 0.3s;position:relative;overflow:hidden;cursor:default;}
  ...
  .mind .pillar-icon{background:var(--lav-pale);}.share .pillar-icon{background:var(--sage-pale);}
  .guide .pillar-icon{background:var(--peach-pale);}.safe .pillar-icon{background:var(--sky-pale);}
  ...
  .how-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:0;max-width:1000px;margin:0 auto;position:relative;}
  .how-step{text-align:center;padding:0 24px;}
  .how-num{width:72px;height:72px;border-radius:50%;background:white;border:2px solid var(--border);display:flex;align-items:center;justify-content:center;font-family:'Fraunces',serif;font-size:28px;font-weight:700;color:var(--sage);margin:0 auto 20px;position:relative;z-index:1;box-shadow:var(--shadow-sm);}
  .how-title{font-family:'Fraunces',serif;font-size:18px;font-weight:600;color:var(--ink);margin-bottom:8px;}
  ...
  .safe-section .section-eyebrow{color:var(--sage-light);}
  ...
  .who-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:20px;max-width:1100px;margin:0 auto;}
  .who-card{background:white;border-radius:var(--r-md);padding:36px 28px;border:1.5px solid var(--border);transition:all 0.3s;text-align:center;}
  .who-card:hover{transform:translateY(-4px);box-shadow:var(--shadow-md);}
  .who-emoji{font-size:40px;margin-bottom:16px;}
  .who-title{font-family:'Fraunces',serif;font-size:20px;font-weight:600;color:var(--ink);margin-bottom:10px;}
  .who-desc{font-size:14px;color:var(--muted);line-height:1.65;}
  .who-tag{display:inline-block;margin-top:16px;background:var(--sage-pale);color:var(--sage);font-size:12px;font-weight:700;padding:5px 14px;border-radius:30px;}
  ...
  .footer-sub-logo{font-family:'Fraunces',serif;font-size:14px;font-weight:600;color:rgba(255,255,255,0.25);}
  .footer-sub-logo span{color:#F0A500;}
  ...
  .modal-close{position:absolute;top:20px;right:20px;background:var(--sand);border:none;width:36px;height:36px;border-radius:50%;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--muted);}
  ...
  .modal-field input,.modal-field select{width:100%;padding:13px 16px;border:2px solid rgba(74,124,89,0.15);border-radius:var(--r-sm);font-size:15px;font-family:inherit;color:var(--ink);outline:none;transition:border-color 0.2s;background:white;}
  .modal-field input:focus,.modal-field select:focus{border-color:var(--sage);}
  .modal-actions{display:flex;gap:12px;margin-top:24px;}
  .btn-modal-primary{flex:1;padding:14px;background:var(--sage);color:white;border:none;border-radius:50px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;}
  ...
  @media(max-width:900px){
    .ss-nav{padding:0 20px;}.ss-nav-links{gap:12px;}.nav-link{display:none;}
    .ss-hero{padding:60px 24px;min-height:auto;}.hero-right{display:none;}
    .section{padding:64px 24px;}.vv-banner{flex-direction:column;padding:40px 28px;}
    .safe-section{padding:64px 24px;}.footer-top{gap:40px;}
    .ss-footer{padding:48px 24px 28px;}.footer-bottom{flex-direction:column;align-items:flex-start;}
  }
`;
...
const HOW_STEPS = [
  { icon: '🌱', title: 'Create your safe space', desc: 'Sign up anonymously. No real name needed. Your identity stays yours.' },
  { icon: '💬', title: 'Share freely', desc: 'Write, express, and explore your thoughts without fear of judgement.' },
  { icon: '🤝', title: 'Connect & be heard', desc: 'Join support circles, talk to peers, and access counsellors when ready.' },
  { icon: '🚀', title: 'Grow & discover', desc: 'Build life skills, find your career path, and become the best version of you.' },
];

const WHO_WE_SERVE = [
  { emoji: '🎒', title: 'School Students', desc: "Class 6–12 students navigating exams, friendships, family expectations, and who they're becoming.", tag: 'Ages 11–17' },
  { emoji: '🎓', title: 'College Students', desc: 'Young adults managing independence, career anxiety, relationships, and the pressure to have it all figured out.', tag: 'Ages 17–25' },
  { emoji: '👨‍👩‍👧', title: 'Parents & Teachers', desc: 'Adults who want to understand, support, and create safe environments for the young people in their lives.', tag: 'Supporters' },
  { emoji: '🏫', title: 'Schools & Institutions', desc: 'Educational bodies seeking a structured, ethical digital wellbeing platform for their student communities.', tag: 'Institutions' },
];

export default function SecretSharz() {
    ...
    <div>
      <nav className="ss-nav">
        ...
        <div className="ss-nav-links">
          ...
          <button className="nav-cta" onClick={() => setModal('join')}>Join Free</button>
        </div>
      </nav>
      <section className="ss-hero">
        ...
        <div className="hero-content">
          ...
          <div className="hero-actions anim-up-3">
            <button className="btn-primary" onClick={() => setModal('join')}>Create My Safe Space 🌱</button>
      ...
      <section className="section" style={{background:'var(--sand)'}}>
        <div className="section-header"><div className="section-eyebrow">What We Offer</div><h2 className="section-h2">Everything a young person <em>actually needs</em></h2><p className="section-p">Four pillars that work together to support your mind, your connections, your future, and your safety.</p></div>
        <div className="pillars-grid">{PILLARS.map((p) => (<div key={p.cls} className={`pillar-card ${p.cls}`}><div className="pillar-icon">{p.icon}</div><div className="pillar-title">{p.title}</div><div className="pillar-desc">{p.desc}</div><div className="pillar-features">{p.features.map((f,i)=><div key={i} className="pillar-feat">{f}</div>)}</div></div>))}</div>
      </section>
      <section className="section">
        <div className="vv-banner">
          <div className="vv-banner-left" style={{position:'relative',zIndex:1,flex:1}}>
            ...
            <button className="btn-vv" onClick={() => setShowVV(true)}>🎓 Start Career Assessment <span style={{fontSize:'18px'}}>→</span></button>
          </div>
          <div style={{position:'relative',zIndex:1,flexShrink:0}}>
            <div className="vv-banner-card">
              <div className="vv-stat"><div className="vv-stat-num">6</div><div className="vv-stat-label">RIASEC Personality Dimensions</div></div>
              <div className="vv-stat"><div className="vv-stat-num">500+</div><div className="vv-stat-label">Indian Colleges in Database</div></div>
      ...
      <section className="section" style={{background:'var(--sand)'}}>
        <div className="section-header"><div className="section-eyebrow">How It Works</div><h2 className="section-h2">Your journey <em>starts here</em></h2><p className="section-p">Four simple steps to a safer, more supported, more directed life.</p></div>
        <div className="how-grid">{HOW_STEPS.map((s,i)=>(<div key={i} className="how-step"><div className="how-num">{s.icon}</div><div className="how-title">{s.title}</div><div className="how-desc">{s.desc}</div></div>))}</div>
      </section>
      <section className="section">
        <div className="section-header"><div className="section-eyebrow">Who We Serve</div><h2 className="section-h2">Built for <em>every young person</em> in India</h2><p className="section-p">From school corridors to college campuses, Secret Sharz is for everyone navigating the beautiful, confusing journey of growing up.</p></div>
        <div className="who-grid">{WHO_WE_SERVE.map((w,i)=>(<div key={i} className="who-card"><div className="who-emoji">{w.emoji}</div><div className="who-title">{w.title}</div><div className="who-desc">{w.desc}</div><div className="who-tag">{w.tag}</div></div>))}</div>
      </section>
      <section className="safe-section">
        <div className="safe-content">
          <div className="section-eyebrow">Your Safety Comes First</div>
          ...
          <div className="safe-promises">{['🔒 Anonymous by default','🤖 AI-moderated content','👁️ Human review team','📞 Crisis escalation','🇮🇳 POCSO-aware'].map((p,i)=>(<div key={i} className="safe-promise">{p}</div>))}</div>
          <div className="crisis-box"><div style={{fontSize:'36px',flexShrink:0}}>🆘</div><div><div className="crisis-title">Need urgent help right now?</div><div className="crisis-desc">Our crisis support connects you to iCall, Vandrevala Foundation, and Snehi helplines — trained professionals available 24/7.</div></div><div className="crisis-number">iCall<br/>9152987821</div></div>
        </div>
      </section>
      <footer className="ss-footer">
        <div className="footer-top">
          <div style={{flex:1,minWidth:'220px'}}><div className="footer-logo">Secret<span>Sharz</span></div><div className="footer-tagline">A safe space for every young person in India to share, heal, and grow — anonymously and without judgement.</div></div>
          <div style={{minWidth:'160px'}}><div className="footer-links-title">Platform</div><a className="footer-link">Mind Space</a><a className="footer-link">Sharz Wall</a><a className="footer-link">Life Guide</a><a className="footer-link">Safe Corner</a></div>
          <div style={{minWidth:'160px'}}><div className="footer-links-title">Our Subsidiary</div><a className="footer-link" onClick={()=>setShowVV(true)}>🎓 VidyaVantage</a><a className="footer-link" onClick={()=>setShowVV(true)}>Career Assessment</a><a className="footer-link" onClick={()=>setShowVV(true)}>College Database</a><a className="footer-link" onClick={()=>setShowVV(true)}>AI Career Report</a></div>
          <div style={{minWidth:'160px'}}><div className="footer-links-title">For Institutions</div><a className="footer-link">Schools & Colleges</a><a className="footer-link">Counsellor Portal</a><a className="footer-link">Parent Resources</a><a className="footer-link">Contact Us</a></div>
        </div>
        <div className="footer-bottom"><div className="footer-copy">© 2025 Secret Sharz. Made with ❤️ for India's youth.</div><div className="footer-sub-logo">VidyaVantage is a subsidiary of Secret<span>Sharz</span></div></div>
      </footer>
      {modal === 'join' && (<div className="modal-overlay" onClick={()=>setModal(null)}><div className="modal" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setModal(null)}>✕</button><div className="modal-icon">🌱</div><h3>Create your safe space</h3><p>Join thousands of young Indians who share, support each other, and grow together — completely free and anonymous.</p><div className="modal-field"><label>Choose a username</label><input placeholder="e.g. QuietStorm, SunflowerDreamer..."/></div><div className="modal-field"><label>Your age group</label><select><option value="">Select your age group</option><option>Under 13</option><option>13–15 years</option><option>16–18 years</option><option>19–22 years</option><option>23–25 years</option></select></div><div className="modal-field"><label>Email (optional)</label><input type="email" placeholder="Leave blank to stay fully anonymous"/></div><div className="modal-actions"><button className="btn-modal-primary" onClick={()=>setModal(null)}>Create My Space 🌱</button><button className="btn-modal-ghost" onClick={()=>setModal(null)}>Cancel</button></div></div></div>)}
      {modal === 'talk' && (<div className="modal-overlay" onClick={()=>setModal(null)}><div className="modal" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setModal(null)}>✕</button><div className="modal-icon">💬</div><h3>You don't have to carry this alone</h3><p>Whether it's a small worry or something really heavy — reaching out is the bravest thing you can do.</p>{[{icon:'🤖',title:'Chat with AI Support',desc:'Available right now.',color:'var(--sage-pale)',textColor:'var(--sage)'},{icon:'👥',title:'Join a Support Circle',desc:'Connect anonymously with peers.',color:'var(--lav-pale)',textColor:'var(--lavender)'},{icon:'📞',title:'Talk to a Real Counsellor',desc:'First session always free.',color:'var(--peach-pale)',textColor:'var(--peach)'},{icon:'🆘',title:'Crisis Support Now',desc:"iCall: 9152987821",color:'#FFF0F0',textColor:'#C0392B'}].map((opt,i)=>(<div key={i} onClick={()=>setModal(null)} style={{display:'flex',alignItems:'center',gap:'16px',background:opt.color,borderRadius:'var(--r-sm)',padding:'16px 18px',marginBottom:'10px',cursor:'pointer'}}><span style={{fontSize:'24px'}}>{opt.icon}</span><div><div style={{fontSize:'14px',fontWeight:'600',color:opt.textColor}}>{opt.title}</div><div style={{fontSize:'12px',color:'var(--muted)'}}>{opt.desc}</div></div></div>))}</div></div>)}
    </div>
  );
}
