...
function normaliseGuardian(data = {}) {
  ...
  Object.entries(guardianRelationships).forEach(([accountId, relationship]) => {
    if (!guardians.some(guardian => guardian.accountId === accountId)) {
  ...
  if (legacyAccountId || legacyName || legacyPhone || legacyEmail) {
    if (!guardians.some(guardian => guardian.accountId === legacyAccountId && guardian.name === legacyName)) {
      guardians.push({ accountId: legacyAccountId, relationship: firstDefined(data.parentRelationship, 'guardian'), name: legacyName, email: legacyEmail, phone: legacyPhone, countryCode: firstDefined(data.contactCountryCode, null), legalGuardian: false, invitationStatus: null, consentStatus: null });
    }
  }
  ...
  [
    ...
    if (!guardians.some(guardian => String(guardian.relationship).toLowerCase() === parent.relationship && guardian.name === parent.name)) {
      guardians.push({ accountId: null, relationship: parent.relationship, name: firstDefined(parent.name, ''), email: firstDefined(parent.email, ''), phone: firstDefined(parent.phone, ''), countryCode: firstDefined(data.contactCountryCode, null), legalGuardian: false, invitationStatus: null, consentStatus: null });
    }
  });
...
export function normalizeStudentRecord(data = {}, id = null) {
  ...
  const canonicalCareer = data.career && typeof data.career === 'object' ? data.career : {};
  const career = {
    ...legacyCareer,
    ...canonicalCareer,
    status: firstDefined(canonicalCareer.status, legacyCareer.status),
    profile: canonicalCareer.profile || legacyCareer.profile,
    riasec: { ...legacyCareer.riasec, ...(canonicalCareer.riasec || {}) },
    roadmap: canonicalCareer.roadmap || legacyCareer.roadmap,
  };
  ...
  return {
    ...
    assessments: asArray(data.assessments || data.assessmentHistory || data.careerAssessment),
    goals: asArray(data.goals),
    relationships: {
      parents: guardians.map(guardian => guardian.accountId).filter(Boolean),
      institutionId: firstDefined(data.academic?.current?.institutionId, data.academic?.institutionId, data.institution?.id, data.institutionId, data.institutionID, null),
      assignments,
    },
    onboarding: {
