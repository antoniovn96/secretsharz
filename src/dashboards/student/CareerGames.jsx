...
function MiniSudoku({ pro }) {
  const solution = [1,2,3,4,3,4,1,2,2,1,4,3,4,3,2,1];
  const [cells, setCells] = useState(() => solution.map((n, i) => i % 3 === 0 || i === 5 ? '' : n));
  const [message, setMessage] = useState('');
  const reset = () => { setCells(solution.map((n, i) => i % 3 === 0 || i === 5 ? '' : n)); setMessage(''); };
  ...
  return <div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,58px)', gap: 5, justifyContent: 'center', margin: '18px 0' }}>{cells.map((v, i) => <input key={i} value={v} onChange={e => setCells(c => c.map((x,j) => j===i ? e.target.value.replace(/[^1-4]/g,'').slice(-1) : x))} disabled={solution[i] !== '' && !(pro && false)} style={{ width: 58, height: 58, textAlign: 'center', fontSize: 22, fontWeight: 900, border: '1px solid #cbd5e1', borderRadius: 8, background: solution[i] === v && v ? '#f8fafc' : '#fff' }} />)}</div><div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}><button onClick={check} style={{ border: 0, borderRadius: 10, padding: '10px 15px', background: '#4f46e5', color: '#fff', fontWeight: 900 }}>Check</button><button onClick={reset} style={{ border: '1px solid #cbd5e1', borderRadius: 10, padding: '10px 15px', background: '#fff', fontWeight: 800 }}>Reset</button></div>{message && <div style={{ textAlign: 'center', marginTop: 12, color: '#475569', fontWeight: 800 }}>{message}</div>}</div>;
}

function WordScramble({ pro }) {
  const words = pro ? ['ARCHITECT','PSYCHOLOGY','ANALYST','DESIGNER'] : ['CAREER','SKILL','VALUE'];
  const [index, setIndex] = useState(0); const [answer, setAnswer] = useState(''); const [message, setMessage] = useState('');
  const word = words[index % words.length]; const scrambled = word.split('').sort(() => 0.5 - Math.random()).join('');
...
function MemoryMatch({ pro }) {
  ...
  const deck = useMemo(() => [...values, ...values].sort(() => Math.random() - 0.5), [values.join(',')]);
  ...
  return <div><div style={{ display:'grid', gridTemplateColumns:`repeat(${pro?4:4}, 58px)`, gap:7, justifyContent:'center', margin:'18px 0' }}>{deck.map((v,i)=>{const shown=open.includes(i)||matched.includes(i); return <button key={i} onClick={()=>click(i)} style={{ width:58,height:58,border:'1px solid #cbd5e1',borderRadius:10,background:shown?'#eef2ff':'#0f172a',fontSize:24,cursor:'pointer' }}>{shown?v:'?'}</button>;})}</div><div style={{textAlign:'center',fontWeight:800,color:'#475569'}}>{matched.length/2} pairs matched</div></div>;
}

function CareerQuest({ pro }) {
  const questions = pro ? [
    ['A school project needs someone to organise the plan and keep everyone on track. What would you enjoy most?', ['Build the timeline','Create the visuals','Interview people','Solve the technical problem']],
    ['You have one weekend to learn a new skill. Which feels most satisfying?', ['Understand how a system works','Make something creative','Help someone improve','Analyse the numbers']],
  ] : [['Which activity sounds most interesting today?', ['Solve a puzzle','Design a poster','Help a friend','Plan a small project']]];
...
export default function CareerGames({ currentUser, onUnlock }) {
  const [selected,setSelected]=useState('sudoku'); const [pro,setPro]=useState(false); const [stats,setStats]=useState({played:0,best:0});
  ...
  const render=()=>selected==='sudoku'?<MiniSudoku pro={pro}/>:selected==='scramble'?<WordScramble pro={pro}/>:selected==='memory'?<MemoryMatch pro={pro}/>:<CareerQuest pro={pro}/>;
