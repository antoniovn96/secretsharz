...
const ParentDetailPanel = ({ parent, isOpen, onClose, onEdit }) => {
  ...
  const status = displayText(parent.status, 'active');
  const activeServices = [...new Set(children.map(child => getPath(child)?.label).filter(Boolean))];
  const institutions = [...new Set(children.map(child => displayText(child.institutionName || child.schoolName, '')).filter(Boolean))];
  ...
  return <><div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-40" onClick={onClose} /><aside className="fixed right-0 top-0 h-full w-full max-w-2xl bg-white shadow-2xl z-50 flex flex-col">
    <header className="p-6 border-b border-slate-100 flex items-start justify-between bg-gradient-to-r from-rose-50 to-white"><div className="flex items-center gap-4 min-w-0">{identity.photoURL ? <img src={identity.photoURL} alt="" className="w-14 h-14 shrink-0 rounded-2xl object-cover shadow-lg" /> : <div className="w-14 h-14 shrink-0 rounded-2xl bg-gradient-to-br from-rose-400 to-pink-500 flex items-center justify-center text-white font-bold text-lg shadow-lg">{displayText(identity.initial, '?')}</div>}<div className="min-w-0"><h2 className="text-xl font-bold text-slate-900 truncate">{displayText(identity.name, 'Parent')}</h2><p className="text-xs text-slate-400 font-mono truncate">{displayText(parent.id, '—')}</p><div className="flex items-center gap-2 mt-2"><span className="px-2.5 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-700">Parent</span><span className={`px-2.5 py-1 rounded-full text-xs font-bold ${status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{status === 'active' ? 'Active' : 'Inactive'}</span></div></div></div><button onClick={onClose} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-xl"><X className="w-6 h-6" /></button></header>
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <section><h3 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2"><Users className="w-4 h-4 text-rose-500" /> Linked Children</h3>{children.length ? <div className="space-y-3">{children.map(child => { const childIdentity = getProfileIdentity(null, child); const path = getPath(child); const professional = getProfessionalName(child, path); return <div key={displayText(child.id, Math.random().toString(36))} className="p-4 bg-slate-50 rounded-xl border border-slate-100"><div className="flex items-center gap-3"><div className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white font-bold shrink-0">{displayText(childIdentity.initial, '?')}</div><div className="min-w-0 flex-1"><p className="font-semibold text-slate-900 truncate">{displayText(childIdentity.name, 'Student')}</p><p className="text-xs text-slate-400">{displayText(child.grade || child.classLevel, 'Class not specified')}{child.schoolName || child.institutionName ? ` • ${displayText(child.schoolName || child.institutionName)}` : ''}</p></div>{path && <span className={`px-2.5 py-1 rounded-lg text-xs font-bold ${path.className}`}>{path.label}</span>}</div><div className="grid grid-cols-2 gap-2 mt-3"><Relationship icon={Building2} label="Institution" value={child.institutionName || child.schoolName || 'Not linked'} /><Relationship icon={Briefcase} label={path?.professional || 'Professional'} value={professional || 'Not assigned'} /></div></div>; })}</div> : <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl text-sm text-slate-500">No student accounts are currently linked to this parent.</div>}</section>
