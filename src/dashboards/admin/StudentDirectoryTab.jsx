...
const StudentDirectoryTab = ({ theme = 'light' }) => {
  ...
  const loadStudents = useCallback(async (background = false) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/students', { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
  ...
  const handleViewDetails = async student => {
    ...
    try {
      const studentId = String(student?.id || '').trim(); if (!studentId) throw new Error('This student record has no valid student ID.');
      const currentUser = auth.currentUser; if (!currentUser) throw new Error('Authentication required to load student details.');
      const token = await currentUser.getIdToken(true);
      ...
      const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload?.error || 'Unable to load the authorized student profile.');
