...
const StudentDirectoryTab = ({ theme = 'light' }) => {
  ...
  const [isAddModalOpen, setIsAddModalOpen] = useState(false); const [deleteTarget, setDeleteTarget] = useState(null); const [editTarget, setEditTarget] = useState(null); const [notification, setNotification] = useState(null);
  ...
  const handleEditSaved = async updatedStudent => { setEditTarget(null); setSelectedStudent(updatedStudent); showNotification(`${text(updatedStudent?.name) || 'Student'} has been updated successfully.`); await loadStudents(true); };
  ...
  return <div className={`space-y-5 ${dark ? 'text-white' : 'text-slate-900'}`}>
    ...
    <div className={`${panel} border rounded-lg shadow-sm overflow-hidden`}><UserDirectoryTable users={students} isLoading={isLoading} onViewDetails={handleViewDetails} onEdit={setEditTarget} onDelete={setDeleteTarget} userRole="student" theme={theme} /></div>
    ...
    <EditStudentModal student={editTarget} isOpen={!!editTarget} onClose={() => setEditTarget(null)} onSaved={handleEditSaved} theme={theme} />
