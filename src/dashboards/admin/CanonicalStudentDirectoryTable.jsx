...
import { Search, Download, Eye, Edit3, Archive, SlidersHorizontal, X, ArrowUpDown, AlertCircle } from 'lucide-react';
...
export default function CanonicalStudentDirectoryTable({ users = [], isLoading = false, filterOptions = {}, onViewDetails, onDelete, onEdit, theme = 'light' }) {
  ...
  const filtered = useMemo(() => {
    ...
    const result = users.filter(user => {
      const values = [
        user?.name, user?.preferredName, user?.email, getStudentId(user), user?.schoolName, user?.institutionName,
        user?.grade, user?.section, user?.academicYear, user?.assignedProfessionalName, getAssessmentCode(user),
      ];
  ...
  const exportCsv = () => {
    ...
    const rows = filtered.map(user => [
      getStudentId(user), user?.name || user?.preferredName || '', user?.email || '', user?.institutionName || user?.schoolName || '', user?.grade || '', user?.section || '', user?.academicYear || '', getStudentPath(user), getAssessmentStatus(user), getAssessmentCode(user), getProfileStatus(user), user?.assignedProfessionalName || 'Unassigned', user?.enrollmentStatus || 'active', user?.lastActivityAt || '',
    ]);
  ...
  return <div className={`rounded-2xl border overflow-hidden ${dark ? 'border-slate-800 bg-[#111827]' : 'border-slate-200 bg-white'}`}>
    <div className={`p-4 border-b ${dark ? 'border-slate-800' : 'border-slate-100'}`}>
      ...
      {showFilters && <div id="student-directory-filters" className={`mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 p-4 rounded-xl ${dark ? 'bg-[#0b1220]' : 'bg-slate-50'}`}>
        ...
        <Filter label="Counsellor / coach" value={counsellor} onChange={setCounsellor} options={['Unassigned', ...(filterOptions.counsellors || [])]} />
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[1250px]">
        ...
        <thead className={dark ? 'bg-[#0b1220]' : 'bg-slate-50'}><tr className="text-left text-[10px] uppercase tracking-wider text-slate-500">
          ...
          <SortableHeader label="Assessment" field="assessment" sort={sort} ascending={ascending} onSort={(field) => { setSort(field); setAscending(sort === field ? !ascending : true); }} />
        ...
        <tbody className={dark ? 'divide-y divide-slate-800' : 'divide-y divide-slate-100'}>
          {!filtered.length ? <tr><td colSpan={8} className="p-14 text-center"><div className="flex flex-col items-center gap-2 text-slate-500"><AlertCircle className="w-5 h-5" /><p className="text-sm font-medium">No students match the current search or filters.</p></div></td></tr> : filtered.map(user => {
            ...
            const meta = pathMeta[getStudentPath(user)] || pathMeta.unassigned;
            const code = getAssessmentCode(user);
            const profileStatus = getProfileStatus(user);
            const attentionFlag = getNeedsAttention(user);
            return <tr key={user.id} onClick={() => onViewDetails?.(user)} className={`cursor-pointer ${attentionFlag ? (dark ? 'bg-amber-950/10 hover:bg-amber-950/20' : 'bg-amber-50/30 hover:bg-amber-50/60') : (dark ? 'hover:bg-white/[0.03]' : 'hover:bg-slate-50')}`}>
              ...
              <td className="px-5 py-4"><div className="text-xs font-semibold text-slate-700">{user?.assignedProfessionalName || 'Unassigned'}</div></td>
              <td className="px-5 py-4"><div className="text-xs text-slate-600">{dateLabel(user.lastActivityAt)}</div></td>
              <td className="px-5 py-4"><div className="flex justify-end gap-1" onClick={e => e.stopPropagation()}><IconButton label="View student" onClick={() => onViewDetails?.(user)}><Eye className="w-4 h-4" /></IconButton><IconButton label="Edit student" onClick={() => onEdit?.(user)}><Edit3 className="w-4 h-4" /></IconButton><IconButton label="Archive student" danger onClick={() => onDelete?.(user)}><Archive className="w-4 h-4" /></IconButton></div></td>
            </tr>;
          })}
        </tbody>
      </table>
    </div>
    <div className={`px-5 py-3 text-[11px] text-slate-500 border-t ${dark ? 'border-slate-800' : 'border-slate-100'}`}>Student ID is the Secret Sharz human-facing identifier. Firebase UID is kept internal. Archive removes a record from active directories without destroying its underlying Firestore document.</div>
  </div>;
}
...
function SortableHeader({ label, field, sort, ascending, onSort }) {
...
function IconButton({ label, onClick, danger = false, children }) {
  return <button type="button" aria-label={label} title={label} onClick={onClick} className={`p-2 rounded-lg ${danger ? 'text-slate-400 hover:text-red-600 hover:bg-red-50' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-100'}`}>{children}</button>;
}
