...
export default function ServiceStudentDirectory({ service = 'career', theme = 'light' }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let response = await fetch('/api/admin/students', { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
      if (response.status === 401) {
        token = await currentUser.getIdToken(true);
        response = await fetch('/api/admin/students', { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
      }
  ...
  const serviceStudents = useMemo(() => students.filter(student => String(student?.path || '').toLowerCase() === meta.path.toLowerCase()), [students, meta.path]);
  const completeProfiles = serviceStudents.filter(student => student?.profileComplete === true || student?.onboardingCompleted === true).length;
  const assessmentComplete = serviceStudents.filter(student => Boolean(student?.riasecCode || student?.careerDNA?.riasec?.code || student?.careerAssessment?.hollandCode?.length)).length;
