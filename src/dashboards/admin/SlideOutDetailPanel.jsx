...
import { X, Mail, Phone, MapPin, Calendar, GraduationCap, Users, BookOpen, Award, Clock, AlertCircle, User, Link, FileText, TrendingUp, Building2, Briefcase, ShieldCheck } from 'lucide-react';
...
const getStudentDocumentId = user => String(user?.studentDocumentId || user?.documentId || user?.internalRecordId || user?.id || '').trim();
const getStudentId = user => String(user?.ssStudentId || user?.studentId || user?.studentID || user?.student_id || '').trim();
...
const SlideOutDetailPanel = ({ user, isOpen, onClose }) => {
  ...
  const path = normalisePath(merged.primary_path || merged.studentTrack || merged.path);
  const pathInfo = PATH_INFO[path];
  const studentId = getStudentId(merged);
  const documentId = getStudentDocumentId(user);

  useEffect(() => {
    if (isOpen && documentId) fetchAdditionalData(documentId);
  }, [isOpen, documentId]);

  const fetchAdditionalData = async recordId => {
    setLoading(true); setAdditionalData(null); setLinkedParents([]); setAssignedProfessional(null); setInstitution(null);
    try {
      const userSnap = await getDoc(doc(db, 'users', recordId));
      const data = userSnap.exists() ? userSnap.data() : {};
      setAdditionalData(data);
      const combined = { ...user, ...data };
      const parentIds = getParentIds(combined);
      const combinedPath = normalisePath(data.primary_path || user?.primary_path || user?.path);
      const professionalId = getProfessionalId(combined, combinedPath);
      const institutionId = data.institutionId || data.institution?.id || data.academic?.current?.institutionId || user?.institutionId || null;
  ...
  const getRIASECScores = () => {
    const scores = merged.riasecScores || merged.careerDNA?.riasec?.scores || {};
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-2xl bg-white shadow-2xl z-50 overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
          <div className="flex items-center gap-4 min-w-0"><ProfileAvatar user={user} data={additionalData || {}} /><div className="min-w-0"><h2 className="text-xl font-bold text-slate-900 truncate">{identity.name}</h2><p className="text-sm text-slate-500 font-mono truncate">{studentId || 'Student ID not assigned'}</p></div></div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <SectionCard title="Relationship Overview" icon={Link}><div className="grid grid-cols-2 gap-3"><RelationshipItem icon={Building2} label="Institution" value={institution?.name || merged.institutionName || merged.schoolName || 'Not linked'} status={institution ? 'Linked' : 'Not linked'} /><RelationshipItem icon={Briefcase} label="Professional" value={assignedProfessional?.name || 'Not assigned'} status={assignedProfessional ? pathInfo.professional : 'Unassigned'} /><RelationshipItem icon={Users} label="Parents / Guardians" value={`${linkedParents.length} linked account${linkedParents.length === 1 ? '' : 's'}`} status={linkedParents.length ? 'Linked' : 'Not linked'} /><RelationshipItem icon={ShieldCheck} label="Service" value={pathInfo.label} status="Active path" /></div></SectionCard>
...
const StatusBadge = ({ status }) => { const config = { active: ['bg-emerald-100', 'text-emerald-700', 'Active'], pending: ['bg-amber-100', 'text-amber-700', 'Pending'], completed: ['bg-blue-100', 'text-blue-700', 'Completed'], none: ['bg-slate-100', 'text-slate-600', 'None'] }; const [bg, text, label] = config[status] || config.none; return <span className={`inline-flex px-3 py-1.5 rounded-lg text-sm font-bold ${bg} ${text}`}>{label}</span>; };
