...
const EditProfessionalModal = ({ professional, isOpen, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (professional && isOpen) {
      setFormData({
        ...
        role: professional.role || professional.professionalRole || 'counsellor',
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'users', professional.id), updates);
      onSuccess?.({ id: professional.id, ...professional, ...updates, updatedAt: new Date() });
      onClose();
    } catch (err) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60]" onClick={() => !isSubmitting && onClose()} />
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl pointer-events-auto max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between p-6 border-b border-slate-100">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center shadow-lg"><UserPlus className="w-5 h-5 text-white" /></div>
              <div><h2 className="text-lg font-bold text-slate-900">Edit Professional</h2><p className="text-sm text-slate-500">Update the directory record</p></div>
            </div>
            <button onClick={onClose} disabled={isSubmitting} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg"><X className="w-5 h-5" /></button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <Field label="Institution / Organisation" icon={Building2}><input name="institutionName" value={formData.institutionName} onChange={handleChange} placeholder="Optional" disabled={isSubmitting} className={inputClass} /></Field>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Field label="Directory Status" icon={ShieldIcon}><select name="status" value={formData.status} onChange={handleChange} disabled={isSubmitting} className={inputClass}><option value="active">Active</option><option value="inactive">Inactive</option></select></Field>
            </div>

            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
              <button type="button" onClick={onClose} disabled={isSubmitting} className="px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-xl">Cancel</button>
              <button type="submit" disabled={isSubmitting} className="px-5 py-2.5 bg-gradient-to-r from-purple-500 to-indigo-500 text-white text-sm font-bold rounded-xl shadow-lg flex items-center gap-2 disabled:opacity-50">{isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving...</> : 'Save Changes'}</button>
            </div>
          </form>
        </div>
      </div>
