      ...
      const response = await fetch('/api/admin/update-student', {
        ...
        body: JSON.stringify({
          ...
          profile: {
            ...
            institution: { name: form.schoolName.trim() },
  ...
  return <>
    ...
    <div className="fixed inset-0 z-[61] flex items-center justify-center p-4 pointer-events-none">
      <div className={`pointer-events-auto w-full max-w-2xl overflow-hidden rounded-2xl border shadow-2xl ${dark ? 'border-slate-700 bg-[#111827] text-white' : 'border-slate-200 bg-white text-slate-900'}`} role="dialog" aria-modal="true" aria-labelledby="edit-student-title">
        <div className="flex items-center justify-between border-b px-5 py-4"><div><h2 id="edit-student-title" className="text-base font-bold">Edit Student Profile</h2><p className="mt-1 text-xs text-slate-500">Updates are written through the canonical student profile adapter.</p></div><button aria-label="Close edit dialog" onClick={onClose} disabled={saving} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button></div>
