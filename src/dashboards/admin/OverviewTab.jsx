    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Live analytics calculated from the current platform data</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard title="Active Users (7d)" value={stats.activeUsers?.value || 0} change={stats.activeUsers?.change} trend={stats.activeUsers?.trend} changeLabel={stats.activeUsers?.changeLabel} icon={Activity} color="bg-gradient-to-br from-purple-500 to-purple-600" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 min-w-0 bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-slate-500 font-medium">Recorded activity by month · last 6 months</p>
            </div>
          </div>
        ...
        <div className="min-w-0 bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          <div className="mb-6">
            <h3 className="text-lg font-bold text-slate-900">Student Distribution</h3>
          ...
          <div className="h-52 min-w-0 w-full relative" style={{ minHeight: 208 }}>
            {totalStudents > 0 ? (
              <ResponsiveContainer width="100%" height={208} minWidth={1} minHeight={208}>
                <PieChart>
                  <Pie data={pathDistribution} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={4} dataKey="value">
                    {pathDistribution.map((entry) => <Cell key={entry.name} fill={PATH_COLORS[entry.name] || '#94A3B8'} />)}
                  </Pie>
                  <Tooltip content={<CustomTooltip />} formatter={(value) => [Number(value).toLocaleString(), 'Students']} />
                </PieChart>
              </ResponsiveContainer>
            ) : <div className="h-full flex items-center justify-center text-sm text-slate-400">No students yet.</div>}
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center"><p className="text-2xl font-bold text-slate-900">{totalStudents.toLocaleString()}</p><p className="text-xs text-slate-500">Students</p></div>
            </div>
          </div>
          <div className="mt-6 space-y-3">
            {pathDistribution.map((item) => (
              <div key={item.name} className="flex items-center justify-between">
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: PATH_COLORS[item.name] || '#94A3B8' }} /><span className="text-sm font-medium text-slate-600">{item.name}</span></div>
                <span className="text-sm font-bold text-slate-900">{Number(item.value || 0).toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between mb-2"><p className="text-slate-400 text-sm font-medium">Student Profile Completion</p><ClipboardCheck className="w-5 h-5 text-emerald-400" /></div>
          <p className="text-3xl font-bold mb-1">{completionRate}%</p>
          <div className="w-full h-2 bg-slate-700 rounded-full mt-3 overflow-hidden"><div className="h-full bg-gradient-to-r from-emerald-400 to-teal-400 rounded-full" style={{ width: `${completionRate}%` }} /></div>
          <p className="text-xs text-slate-500 mt-3">{profileComplete.toLocaleString()} of {totalStudents.toLocaleString()} student profiles complete</p>
        </div>
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between mb-2"><p className="text-slate-400 text-sm font-medium">Wellbeing Check-ins</p><HeartPulse className="w-5 h-5 text-teal-400" /></div>
          <p className="text-3xl font-bold mb-1">{Number(counts.moodCheckins || 0).toLocaleString()}</p>
          <p className="text-xs text-slate-500 mt-3">Mood entries recorded across student profiles</p>
        </div>
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between mb-2"><p className="text-slate-400 text-sm font-medium">Career Roadmaps</p><Map className="w-5 h-5 text-indigo-400" /></div>
          <p className="text-3xl font-bold mb-1">{Number(counts.careerRoadmaps || 0).toLocaleString()}</p>
          <p className="text-xs text-slate-500 mt-3">Published roadmap records</p>
        </div>
      </div>

      {(limitations.assessmentHistory || limitations.sessionRating || limitations.responseTime) && (
        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
          <p className="text-sm font-bold text-slate-700 mb-2">Data integrity notes</p>
          <ul className="space-y-1 text-xs text-slate-500">
            {limitations.assessmentHistory && <li>• {limitations.assessmentHistory}</li>}
            {limitations.sessionRating && <li>• {limitations.sessionRating}</li>}
            {limitations.responseTime && <li>• {limitations.responseTime}</li>}
          </ul>
        </div>
      )}
    </div>
