...
const InstitutionDashboard = ({ currentUser, userData, onBack, onLogout }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Unable to load institution dashboard.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <header className="sticky top-0 z-20 bg-white/95 backdrop-blur border-b border-slate-200">
        <div className="max-w-[1500px] mx-auto px-5 md:px-8 h-[72px] flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={onBack} title="Back to Secret Sharz" className="w-10 h-10 rounded-xl bg-emerald-500 text-white flex items-center justify-center shadow-sm hover:bg-emerald-600"><ExternalLink className="w-4 h-4" /></button>
            <div className="min-w-0"><div className="text-[10px] uppercase tracking-[0.16em] font-black text-slate-400">Secret Sharz · Institution</div><h1 className="font-black text-lg truncate">{institution.name || userData?.institutionName || 'Institution Dashboard'}</h1></div>
          </div>
