...
const CareerRoadmapView = ({ studentId }) => {
  ...
  useEffect(() => {
    ...
    const loadStudent = async () => {
      ...
      try {
        ...
        if (latest?.id) setRoadmapId(latest.id);
        if (latest?.status === 'Published' || latest?.status === 'Draft') setStatus(latest.status);
      } catch (error) {
        if (cancelled) return;
  ...
  const handleSave = async (nextStatus) => {
    if (!Object.values(roadmap).some(value => value.trim())) {
      setMessage({ type: 'error', text: 'Please complete at least one roadmap phase before saving.' });
      return;
    }
    setSaving(true); setMessage(null);
    try {
      const response = await saveCareerRoadmap(studentId, roadmap, nextStatus);
      setRoadmapId(response?.roadmap?.id || roadmapId);
      setStatus(nextStatus);
      setMessage({ type: 'success', text: nextStatus === 'Published' ? 'Career roadmap securely published.' : 'Career roadmap saved as draft.' });
    } catch (error) {
      console.error('[CareerRoadmap] save failed:', error);
      setMessage({ type: 'error', text: error.message || 'The roadmap could not be saved. Please try again.' });
  ...
  return <div className="min-h-screen bg-slate-50 pb-20 font-sans text-slate-800">
    <header className="sticky top-0 z-50 flex items-center justify-between border-b border-slate-200 bg-white px-5 py-4 md:px-8"><button onClick={() => navigate(`/provider/career/case/${studentId}`)} className="font-semibold text-slate-500 hover:text-indigo-600">← Back to Student Case</button><div className="flex items-center gap-3"><span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider ${status === 'Published' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{status}</span><span className="text-xs font-bold uppercase tracking-wider text-indigo-600">Career Roadmap Builder</span></div></header>
    <main className="mx-auto max-w-5xl space-y-6 px-5 py-8 md:px-8">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8"><div className="mb-8 border-b border-slate-100 pb-6"><h2 className="text-2xl font-black text-slate-900">🗺️ Strategic Career Roadmap</h2><p className="mt-2 text-sm leading-relaxed text-slate-500">Build an actionable five-phase plan that translates the student's profile into concrete career-development steps.</p></div><div className="mb-6 rounded-xl bg-slate-50 p-4 text-sm text-slate-600"><strong className="text-slate-800">Workflow:</strong> save your working version as a Draft. Publish only when the roadmap is reviewed and ready to be released.</div><form onSubmit={event => { event.preventDefault(); handleSave('Draft'); }} className="space-y-8">{phases.map(([key,title,badge,description]) => <div key={key}><label className="flex flex-wrap items-center gap-2 text-lg font-black text-slate-800">{title}<span className="rounded bg-indigo-50 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-indigo-600">{badge}</span></label><p className="mt-1 text-sm text-slate-500">{description}</p><textarea name={key} value={roadmap[key]} onChange={event => setRoadmap(current => ({ ...current, [key]: event.target.value }))} rows={4} className="mt-3 w-full resize-y rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-2 focus:ring-indigo-100" /></div>)}<div className="flex flex-col gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-end"><button disabled={saving} type="submit" className="rounded-xl border border-slate-300 bg-white px-7 py-3.5 text-sm font-black text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">{saving ? 'Saving…' : 'Save Draft'}</button><button disabled={saving} type="button" onClick={() => handleSave('Published')} className="rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 px-7 py-3.5 text-sm font-black text-white shadow-lg transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60">{saving ? 'Publishing…' : '🚀 Publish Career Roadmap'}</button></div></form></section>
    </main>
  </div>;
};
