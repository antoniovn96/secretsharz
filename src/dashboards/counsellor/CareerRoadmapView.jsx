...
const CareerRoadmapView = ({ studentId, currentUser }) => {
  ...
  useEffect(() => {
    const loadStudent = async () => {
      ...
      try {
        ...
        if (!snapshot.exists()) { setMessage({ type: 'error', text: 'Student master record not found.' }); return; }
        const data = snapshot.data();
        const isAdmin = currentUser?.email?.toLowerCase() === 'antonio.antonio.noronha@gmail.com';
        if (!isAdmin && data?.assignedStaff?.careerId !== providerId) { setAccessDenied(true); return; }
        setStudentData(data);
      } catch (error) {
  ...
  const handleSave = async event => {
    ...
    try {
      await addDoc(collection(db, 'students', studentId, 'career_roadmaps'), { providerId, status: 'Published', phases: { ...roadmap }, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
      setMessage({ type: 'success', text: 'Career roadmap securely published.' });
    } catch (error) { console.error('Error publishing career roadmap:', error); setMessage({ type: 'error', text: 'The roadmap could not be published. Please try again.' }); }
  ...
  if (accessDenied) return <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6"><div className="max-w-lg rounded-2xl border border-rose-200 bg-white p-8 text-center shadow-sm"><div className="text-4xl mb-3">🔒</div><h1 className="text-xl font-black text-slate-900">Roadmap access restricted</h1><p className="mt-2 text-sm leading-6 text-slate-600">This student is not assigned to your career-guidance account.</p><button onClick={() => navigate('/provider/career')} className="mt-5 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white">Back to My Students</button></div></div>;
  ...
  return <div className="min-h-screen bg-slate-50 pb-20 font-sans text-slate-800">
    <header className="sticky top-0 z-50 flex items-center justify-between border-b border-slate-200 bg-white px-5 py-4 shadow-sm md:px-8"><button onClick={() => navigate(`/provider/career/case/${studentId}`)} className="font-semibold text-slate-500 hover:text-indigo-600">← Back to Student Case</button><span className="text-xs font-bold uppercase tracking-wider text-indigo-600">Career Roadmap Builder</span></header>
