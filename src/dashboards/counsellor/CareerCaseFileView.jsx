...
function formatDate(value) { if (!value) return '—'; const date = value?.toDate ? value.toDate() : new Date(value); return Number.isNaN(date.getTime()) ? '—' : date.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }); }
...
const CareerCaseFileView = ({ studentId, currentUser }) => {
  ...
  const loadCase = async () => {
    ...
    try {
      ...
      if (!snapshot.exists()) { setStudent(null); setMessage({ type: 'error', text: 'Student master record not found.' }); return; }
      const data = snapshot.data();
      const isAdmin = currentUser?.email?.toLowerCase() === 'antonio.antonio.noronha@gmail.com';
      if (!isAdmin && data?.assignedStaff?.careerId !== providerId) {
        setStudent(null);
        setMessage({ type: 'error', text: 'This student is not assigned to your career-guidance account.' });
        return;
      }
  ...
  useEffect(() => { loadCase(); }, [studentId, providerId]);
  ...
  const handleSaveNote = async event => {
    ...
    setSavingNote(true); setMessage(null);
    try {
      await addDoc(collection(db, 'students', studentId, 'career_notes'), { providerId, type: note.type, title: note.title.trim() || 'Career counselling note', content: note.content.trim(), followUpDate: note.followUpDate || null, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
      setNote({ type: 'session', title: '', content: '', followUpDate: '' }); await loadCase(); setActiveTab('notes'); setMessage({ type: 'success', text: 'Counsellor note saved securely.' });
    } catch (error) { console.error('Error saving career note:', error); setMessage({ type: 'error', text: 'The note could not be saved. Please try again.' }); }
