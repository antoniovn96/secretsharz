...
export default function CounsellingCaseViewSecure({ studentId, currentUser }) {
  ...
  const load = async () => {
    ...
    const [caseData, sessionData] = await Promise.all([
      api(`/api/professional/counselling/case?studentId=${encoded}`),
      api(`/api/professional/counselling/session?studentId=${encoded}`),
    ]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try { await load(); }
      catch (error) { if (active) setDenied(error.message.includes('authorised') || error.message.includes('Authentication')); }
  ...
  const save = async (event) => {
    ...
    try {
      await api(`/api/professional/counselling/session?studentId=${encodeURIComponent(studentId)}`, { method: 'POST', body: JSON.stringify({ studentId, soap }) });
      await load();
  ...
  return <div className="min-h-screen bg-slate-50 pb-20">
    ...
    <div className="mx-auto mt-8 max-w-5xl space-y-8 px-6">
      <div className="flex flex-col justify-between gap-6 rounded-2xl border border-slate-200 bg-white p-8 shadow-sm md:flex-row md:items-center"><div><h1 className="text-3xl font-extrabold text-slate-900">{student?.name || 'Unknown Student'}</h1><div className="mt-2 flex gap-4 text-sm font-semibold text-slate-500"><span>🏫 {student?.schoolName || 'N/A'}</span><span>🎓 {student?.grade || 'N/A'}</span></div></div><div className="rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm"><p className="font-bold text-slate-800">Counselling Case</p><p className="text-slate-500">Professional view only</p></div></div>
      <section className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm"><h2 className="mb-6 text-xl font-bold text-slate-800">Clinical Session History</h2>{sessions.length === 0 ? <div className="rounded-xl bg-slate-50 p-6 text-sm font-semibold text-slate-500">No clinical sessions recorded yet.</div> : <div className="space-y-4">{sessions.map(session => <article key={session.id} className="rounded-xl border border-slate-200 p-5"><div className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400">SOAP · {session.createdAt ? new Date(session.createdAt).toLocaleString() : 'Recorded session'}</div><div className="grid gap-3 text-sm"><p><b>S:</b> {session.soap?.subjective || '—'}</p><p><b>O:</b> {session.soap?.objective || '—'}</p><p><b>A:</b> {session.soap?.assessment || '—'}</p><p><b>P:</b> {session.soap?.plan || '—'}</p></div></article>)}</div>}</section>
