...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Videos & events', description: 'Coming from the official Secret Sharz channel and events library.', path: '/resources' },
    ],
  },
  {
    label: 'About',
  ...
  return (
    <>
        ...
        <div className="ss-header-inner">
                        ...
                        <div className="ss-nav-panel-inner">
                          {group.items.map((item) => (
                            <NavAnchor
                              ...
                              className={`ss-nav-card ${currentPath === item.path || currentPath.startsWith(`${item.path}/`) ? 'is-current' : ''}`}
                            />
                          ))}
                        </div>
                      </div>
          ...
          <div className="ss-header-actions">
            <button
              ...
              title={reducedMotion ? 'Motion is reduced. Activate to restore motion.' : 'Reduce motion'}
            >
              {reducedMotion ? 'Motion reduced' : 'Accessibility'}
            </button>
            ...
            {currentUser && (
              <div className="ss-alert-wrap" ref={alertsRef}>
                <button
                  ...
                  aria-controls="ss-notifications"
                >
      ...
      {mobileOpen && (
        <div className="ss-mobile-layer">
          ...
          <aside
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="ss-mobile-title"
          >
            <div className="ss-mobile-heading">
              <span id="ss-mobile-title">Secret Sharz</span>
              <button className="ss-icon-btn" type="button" onClick={() => setMobileOpen(false)} aria-label="Close navigation menu">×</button>
            </div>
                ...
                {NAV_GROUPS.map((group) => (
                  <li key={group.label} className="ss-mobile-group">
                    <details open={activeGroup === group.label}>
                      ...
                      <ul>
                        {group.items.map((item) => (
                          <li key={`${group.label}-${item.path}-${item.label}`}>
                            <NavAnchor item={item} navigate={navigate} onNavigate={() => setMobileOpen(false)} className="ss-mobile-sublink" />
                          </li>
                        ))}
                      </ul>
                    </details>
            ...
            <button className="ss-mobile-link" type="button" onClick={toggleReducedMotion} aria-pressed={reducedMotion}>
              {reducedMotion ? '✓ Motion reduced' : 'Reduce motion'}
            </button>
            {currentUser ? (
              <>
                {isAdmin && <button className="ss-mobile-link" type="button" onClick={() => go('/admin')}>Admin</button>}
                <button className="ss-mobile-link" type="button" onClick={() => go('/dashboard')}>My space</button>
                <button className="ss-mobile-link danger" type="button" onClick={() => { setMobileOpen(false); handleLogout?.(); }}>Sign out</button>
              </>
            ) : (
              <button className="ss-mobile-join" type="button" onClick={() => go('/auth')}>Join Secret Sharz</button>
            )}
          </aside>
        </div>
      )}
      ...
      <style>{`
        ...
        .ss-header{position:sticky;top:0;z-index:3000;background:rgba(253,252,250,.97);border-bottom:1px solid rgba(30,40,32,.12);backdrop-filter:blur(16px)}
        .ss-header-inner{max-width:1440px;margin:0 auto;min-height:72px;padding:10px 28px;display:flex;align-items:center;gap:18px}
        .ss-brand{border-radius:12px;padding:4px;display:flex;align-items:center;flex:0 0 auto}
        .ss-brand-logo{display:block;height:42px;width:auto}
        .ss-desktop-nav{margin-left:auto}
        .ss-nav-list{display:flex;align-items:center;gap:2px;list-style:none;margin:0;padding:0}
        .ss-nav-item{position:relative}
        .ss-nav-trigger{min-height:44px;padding:10px 11px;border:0;background:transparent;color:#33443a;border-radius:10px;font:inherit;font-size:14px;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:6px}
        .ss-nav-trigger:hover,.ss-nav-trigger.is-active,.ss-nav-trigger[aria-expanded="true"]{background:#EBF4EE;color:#24583a}
        .ss-nav-chevron{font-size:14px;line-height:1;transition:transform .18s ease}.ss-nav-trigger[aria-expanded="true"] .ss-nav-chevron{transform:rotate(180deg)}
        .ss-nav-panel{position:absolute;left:50%;top:calc(100% + 8px);transform:translateX(-50%);width:min(620px,calc(100vw - 32px));background:#FDFCFA;border:1px solid #D7DED9;border-radius:18px;box-shadow:0 22px 70px rgba(30,40,32,.16);padding:10px;z-index:3100}
        .ss-nav-panel-inner{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px}
        .ss-nav-card{display:block;text-decoration:none;color:#24342b;padding:14px;border-radius:13px}.ss-nav-card:hover,.ss-nav-card:focus-visible,.ss-nav-card.is-current{background:#EBF4EE;color:#24583a}.ss-nav-card span{display:block;font-weight:800;font-size:14px}.ss-nav-card small{display:block;color:#66736b;font-size:12px;line-height:1.45;margin-top:4px}.ss-nav-card:hover small,.ss-nav-card:focus-visible small{color:#42554a}
        .ss-header-actions{display:flex;align-items:center;gap:7px}
        .ss-accessibility-btn{min-height:44px;border:1px solid #9CAAA0;background:#fff;color:#26372d;border-radius:10px;padding:9px 11px;font-size:12px;font-weight:800;cursor:pointer;white-space:nowrap}
        .ss-icon-btn{position:relative;width:44px;height:44px;border-radius:11px;border:1px solid #CBD5CE;background:#fff;color:#20352a;cursor:pointer;font-size:18px}
        ...
        .ss-dashboard-btn,.ss-join-btn{min-height:44px;border:0;background:#2E6B4A;color:#fff;border-radius:12px;padding:10px 15px;font-weight:800;cursor:pointer;white-space:nowrap}.ss-dashboard-btn:hover,.ss-join-btn:hover{background:#214F37}
        .ss-admin-link,.ss-signout-btn,.ss-text-btn{min-height:44px;font:inherit;border:0;background:transparent;cursor:pointer}.ss-admin-link{color:#8A5B00;font-size:13px;font-weight:800;padding:8px}.ss-signout-btn{color:#7A2E2E;font-size:13px;font-weight:750;padding:8px}
        ...
        @media(max-width:980px){.ss-desktop-nav,.ss-header-actions{display:none}.ss-menu-btn{display:block;margin-left:auto}.ss-header-inner{min-height:68px;padding:10px 16px}}
        @media(max-width:640px){.ss-brand-logo{height:38px}.ss-mobile-menu{padding:18px}.ss-nav-panel{width:calc(100vw - 24px)}.ss-nav-panel-inner{grid-template-columns:1fr}}
        @media(prefers-reduced-motion:reduce){.ss-nav-chevron{transition:none}.ss-nav-card,.ss-mobile-sublink{transition:none}}
        @media(max-width:980px){html[data-reduced-motion="true"] *{animation-duration:.001ms!important;animation-iteration-count:1!important;transition-duration:.001ms!important;scroll-behavior:auto!important}}
      `}</style>
    </>
  );
}
