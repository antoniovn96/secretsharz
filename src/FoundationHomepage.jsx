  ...
  return (
    <div className="ss-foundation-page">
      <main id="main-content" tabIndex="-1">
        <section className="ss-fh-hero" aria-labelledby="ss-fh-title">
          ...
          <div className="ss-fh-hero-grid">
            ...
            <div className="ss-fh-hero-art" aria-label="Four ways to begin with Secret Sharz">
        ...
        <section className="ss-fh-video" aria-labelledby="ss-fh-video-title">
          <div className="ss-fh-section-heading">
            ...
            <p>A real Secret Sharz gathering. More events and recordings can be added here as they happen.</p>
          </div>
          <div className="ss-fh-video-frame">
            <iframe
              src="https://www.youtube.com/embed/GtUtrEgrF24"
              title="Secret Sharz event recording"
              loading="lazy"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
              allowFullScreen
            />
          </div>
          <p className="ss-fh-video-note">If captions are available for the recording, turn them on in the YouTube player. A full transcript will be provided alongside Secret Sharz-produced recordings where available.</p>
        </section>
      ...
      <style suppressHydrationWarning>{`
        ...
        .ss-fh-kicker,.ss-fh-eyebrow{font-size:13px;text-transform:uppercase;letter-spacing:.14em;font-weight:850;color:var(--fh-green);margin:0 0 16px}
        .ss-fh-kicker span{display:inline-block;margin-right:8px;animation:ss-pulse 2.6s ease-in-out infinite}
        .ss-fh-hero h1{font-family:Fraunces,serif;font-size:clamp(46px,6vw,78px);line-height:1.04;letter-spacing:-.045em;max-width:780px;margin:0 0 24px;font-weight:700}
        .ss-fh-hero h1 em{color:var(--fh-green);font-style:italic}
        .ss-fh-lead{font-size:clamp(18px,2vw,22px);line-height:1.65;color:var(--fh-text);max-width:720px;margin:0 0 32px}
        .ss-fh-actions{display:flex;gap:12px;flex-wrap:wrap;align-items:center}
        .ss-fh-primary,.ss-fh-secondary{min-height:50px;border-radius:14px;padding:13px 21px;cursor:pointer;font-weight:800;border:2px solid transparent}
        .ss-fh-primary{background:var(--fh-green);color:#fff;box-shadow:0 10px 26px rgba(46,107,74,.18)}
        .ss-fh-primary:hover{background:#214F37;transform:translateY(-2px)}
        .ss-fh-secondary{background:#fff;color:var(--fh-deep);border-color:#B8C6BD}
        .ss-fh-secondary:hover{border-color:var(--fh-green);background:var(--fh-pale)}
        .ss-fh-primary:focus-visible,.ss-fh-secondary:focus-visible,.ss-fh-path-card:focus-visible,.ss-fh-service-card button:focus-visible,.ss-fh-text-link:focus-visible{outline:3px solid #1F6B46;outline-offset:4px}
        .ss-fh-note{font-size:13px;color:var(--fh-muted);margin-top:18px}
        .ss-fh-orb{position:absolute;border-radius:50%;filter:blur(2px);pointer-events:none}
        .ss-fh-orb-one{width:480px;height:480px;right:-170px;top:-120px;background:radial-gradient(circle,rgba(94,145,108,.16),transparent 68%);animation:ss-drift 13s ease-in-out infinite}
        .ss-fh-orb-two{width:320px;height:320px;left:30%;bottom:-230px;background:radial-gradient(circle,rgba(206,184,119,.13),transparent 68%);animation:ss-drift 17s ease-in-out infinite reverse}
        .ss-fh-hero-art{min-height:460px;position:relative;display:flex;align-items:center;justify-content:center}
        .ss-fh-art-card{border:1px solid rgba(30,60,45,.12);background:rgba(255,255,255,.9);box-shadow:0 24px 70px rgba(26,47,36,.13);backdrop-filter:blur(14px);border-radius:28px}
        .ss-fh-art-main{width:min(390px,90%);padding:42px;display:flex;flex-direction:column;gap:12px;position:relative;z-index:2;animation:ss-float 7s ease-in-out infinite}
        .ss-fh-art-label{font-size:12px;text-transform:uppercase;letter-spacing:.12em;color:var(--fh-green);font-weight:850}
        .ss-fh-art-main strong{font-family:Fraunces,serif;font-size:32px;line-height:1.15}
        .ss-fh-art-main span:last-child{color:var(--fh-muted);line-height:1.55}
        .ss-fh-art-float{position:absolute;padding:15px 20px;border-radius:18px;font-weight:850;color:var(--fh-deep);z-index:3}
        .ss-fh-art-float.one{top:35px;right:20px;background:#E9F3EC;transform:rotate(5deg);animation:ss-float 6s 1s ease-in-out infinite}
        .ss-fh-art-float.two{bottom:85px;left:20px;background:#F2EEE3;transform:rotate(-5deg);animation:ss-float 7s 2s ease-in-out infinite}
        .ss-fh-art-float.three{bottom:25px;right:55px;background:#E9EEF1;transform:rotate(4deg);animation:ss-float 8s 1.5s ease-in-out infinite}
        .ss-fh-pathways,.ss-fh-services,.ss-fh-video{padding:105px 6vw}
        .ss-fh-pathways{background:#fff}
        .ss-fh-section-heading{max-width:760px;margin:0 auto 48px;text-align:center}
        .ss-fh-section-heading h2,.ss-fh-share h2,.ss-fh-human h2,.ss-fh-final h2{font-family:Fraunces,serif;font-size:clamp(34px,4.2vw,55px);line-height:1.1;letter-spacing:-.035em;margin:0 0 18px}
        .ss-fh-section-heading>p:last-child{color:var(--fh-text);font-size:18px;line-height:1.65;margin:0}
        .ss-fh-path-grid{width:min(1200px,100%);margin:auto;display:grid;grid-template-columns:repeat(4,1fr);gap:18px}
        .ss-fh-path-card{text-align:left;border:1px solid var(--fh-line);background:#FDFCFA;border-radius:22px;padding:28px;min-height:235px;cursor:pointer;position:relative;transition:transform .22s ease,box-shadow .22s ease,border-color .22s ease}
        .ss-fh-path-card:hover{transform:translateY(-5px);box-shadow:0 18px 45px rgba(30,55,42,.10);border-color:#AFC1B5}
        .ss-fh-path-card.care{border-top:4px solid #5E8E6E}.ss-fh-path-card.understand{border-top:4px solid #6D8195}.ss-fh-path-card.future{border-top:4px solid #A38355}.ss-fh-path-card.connect{border-top:4px solid #7D7695}
        .ss-fh-path-arrow{position:absolute;right:22px;top:20px;font-size:22px;color:var(--fh-green)}
        .ss-fh-path-title{display:block;font-size:20px;font-weight:850;max-width:240px;padding-top:20px;margin-bottom:12px}
        .ss-fh-path-text{display:block;color:var(--fh-muted);line-height:1.6;font-size:15px}
        .ss-fh-share{background:var(--fh-deep);color:#fff;padding:110px 6vw;display:grid;grid-template-columns:minmax(0,.9fr) minmax(420px,1.1fr);gap:90px;align-items:center}
        .ss-fh-share .ss-fh-eyebrow,.ss-fh-eyebrow.light{color:#B9D6C2}.ss-fh-share h2{color:#fff}.ss-fh-share-copy>p:not(.ss-fh-eyebrow){color:rgba(255,255,255,.78);font-size:18px;line-height:1.7;max-width:610px}
        .ss-fh-text-link{margin-top:25px;border:0;background:transparent;color:#fff;font-weight:850;padding:8px 0;cursor:pointer;border-bottom:2px solid #9BC5A8}
        .ss-fh-share-list{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:0}
        .ss-fh-share-step{display:grid;grid-template-columns:58px 1fr 8px;gap:18px;align-items:start;min-height:105px;position:relative}
        .ss-fh-share-letter{width:48px;height:48px;border-radius:50%;background:#2E6B4A;border:1px solid #76A88A;display:flex;align-items:center;justify-content:center;font-weight:900;color:#fff}
        .ss-fh-share-step strong{display:block;font-size:20px;margin-bottom:5px}.ss-fh-share-step small{display:block;color:rgba(255,255,255,.68);font-size:14px;line-height:1.5}.ss-fh-share-line{position:absolute;left:24px;top:52px;height:53px;width:1px;background:rgba(185,214,194,.35)}
        .ss-fh-service-grid{width:min(1200px,100%);margin:auto;display:grid;grid-template-columns:repeat(4,1fr);gap:18px}
        .ss-fh-service-card{border:1px solid var(--fh-line);border-radius:22px;padding:30px;background:#fff;min-height:250px;display:flex;flex-direction:column}
        .ss-fh-service-card h3{font-family:Fraunces,serif;font-size:28px;margin:0 0 12px}.ss-fh-service-card p{color:var(--fh-muted);line-height:1.65;margin:0 0 25px}.ss-fh-service-card button{margin-top:auto;width:max-content;border:0;background:none;color:var(--fh-green);font-weight:850;padding:8px 0;cursor:pointer}
        ...
        .ss-fh-video{background:#fff}.ss-fh-video-frame{width:min(1000px,100%);margin:0 auto;border-radius:24px;overflow:hidden;background:#17231d;box-shadow:0 20px 60px rgba(30,40,32,.12);aspect-ratio:16/9}.ss-fh-video-frame iframe{width:100%;height:100%;border:0}.ss-fh-video-note{width:min(1000px,100%);margin:14px auto 0;color:var(--fh-muted);font-size:13px;line-height:1.6}
